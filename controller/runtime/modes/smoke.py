...
class SmokeMode(ControlMode):
    ...
    def on_metrics_stamped(self):
        # Shared with Startup/Reignite. The cycle settings are recorded from
        # here, not from setup(): setup() runs before run() stamps the row, and
        # anything written to self.state.metrics there is discarded when the
        # stamped row replaces it. See
