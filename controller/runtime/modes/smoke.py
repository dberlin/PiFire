...
class SmokeMode(ControlMode):
    ...
    def _init_smoke_cycle(self):
        ...
        # do NOT write_metrics() here: setup() (this method's caller) runs BEFORE
        # ControlMode.run() stamps a fresh metrics row (write_metrics(new_metric=
        ...
        # -- missing 'starttime' and every other column. write_metrics() would hit
        ...
        # at the next Stop transition. The reassignment below the stamp already
        # replaces self.state.metrics wholesale, so this write's effect was being
        # discarded anyway; dropping it is a pure bugfix, not a behavior change.
