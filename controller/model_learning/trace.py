...
def _allocation_result(payload: AllocationPayload | None) -> AllocationResult | None:
    if payload is None:
        return None
    return AllocationResult(
        normalized_combustion_load=payload.normalized_combustion_load,
        auger_duty=payload.requested_auger_duty,
        fan_duty=payload.requested_fan_duty,
        u_max=payload.u_max,
        fan_min_pct=payload.fan_min_pct,
        fan_max_pct=payload.fan_max_pct,
        fan_enabled=payload.fan_enabled,
        auger_clamp_reason=payload.auger_clamp_reason,
        fan_clamp_reason=payload.fan_clamp_reason,
    ...
    for payload in payloads:
        ...
        frames.append(
            FrameObservation(
                ...
                output_source=cast(OutputSource, payload.output_source).value,
                lid_open=cast(bool, payload.lid_open),
                safety_inhibited=cast(bool, payload.safety_inhibited),
                manual_override=cast(bool, payload.manual_override),
                stale=cast(bool, payload.stale),
                ...
                calibration_command_revision=payload.calibration_command_revision,
                calibration_command_action=payload.calibration_command_action,
                calibration_cancellation_reason=payload.calibration_cancellation_reason,
                baseline_allocation=_allocation_result(payload.baseline_allocation),
                combined_allocation=_allocation_result(payload.combined_allocation),
                calibration_status=payload.calibration_status,
                cancellation_command_revision=payload.cancellation_command_revision,
                cancellation_command_action=payload.cancellation_command_action,
            )
        )
...
def calibration_samples(records: Iterable[ControlTraceRecord]) -> tuple[CalibrationSample, ...]:
    ...
    if exact:
        exact_frames = _exact_observations(exact, _allocation_payloads(trace))
        start_s = exact_frames[0].frame_end_s if exact_frames else 0.0
        return tuple(
            CalibrationSample(
    ...
    for revision, revision_outputs in complete_outputs.items():
        ...
        for output_index, output in revision_outputs:
            ...
            if not math.isclose(_realized_combustion_load(output), delivered_load, rel_tol=0, abs_tol=1e-6):
                raise TraceSelectionError(
    ...
    def realized_load(revision_outputs: list[tuple[int, AppliedOutputPayload]]) -> float:
        if len(revision_outputs) == 1:
            return _realized_combustion_load(revision_outputs[0][1])
        duration_ms = sum(output.interval_end_ms - output.interval_start_ms for _, output in revision_outputs)
        return (
            sum(
                _realized_combustion_load(output) * (output.interval_end_ms - output.interval_start_ms)
