...
def create_fuzzy_pickle(plot=False):
    ...
    current = ctrl.Antecedent(
        np.arange(0, 600, 1), "current"
    )  # The range of temperatures that we expect are possible/reasonable
    rate_of_change = ctrl.Antecedent(
        np.arange(0, 1, 0.01), "rate_of_change"
    )  # The range rate of change of temperature since last
    cycleratio = ctrl.Consequent(
        np.arange(0, 1, 0.01), "cycleratio"
    )  # The ratio of Auger On / Auger Off time within the time cycle (default 20s cycle)
    ...
    rules.append(ctrl.Rule(delta["pMed"] & current["Low"] & rate_of_change["Low"], cycleratio["Max"]))
    rules.append(
        ctrl.Rule(
            delta["pMed"]
            & (current["MediumLow"] | current["MediumHigh"] | current["High"])
            & (rate_of_change["Medium"] | rate_of_change["Low"]),
            cycleratio["Max"],
        )
    )
    rules.append(
        ctrl.Rule(
            delta["pMed"] & (current["MediumLow"] | current["MediumHigh"] | current["High"]) & rate_of_change["High"],
            cycleratio["Medium"],
        )
    )
    rules.append(
        ctrl.Rule(
            delta["pMed"] & current["Low"] & (rate_of_change["Medium"] | rate_of_change["High"]), cycleratio["Medium"]
        )
    )

    rules.append(ctrl.Rule(delta["pMed"] & current["Low"], cycleratio["Medium"]))
    rules.append(
        ctrl.Rule(delta["pMed"] & (current["MediumLow"] | current["MediumHigh"] | current["High"]), cycleratio["Max"])
    )
