...
from controller.model_learning.contracts import AffinePrediction, FloatArray

DELAY_STATES = 8
PREDICTION_STEP_S = 25.0
PUBLIC_STATE_WIDTH = 10
_KELVIN = 273.15
...
class GreyBoxPredictionAdapter:
    ...
    def __post_init__(self) -> None:
        ...
        size = state.size
        if size != PUBLIC_STATE_WIDTH:
            raise ValueError("grey prediction origin must contain exactly ten states")
        ...
        if temperature_index != DELAY_STATES:
            raise ValueError("grey prediction origin must use exactly eight delay states")
    ...
    def from_estimator(cls, estimator: Any, *, config: Mapping[str, object]) -> GreyBoxPredictionAdapter:
        ...
        _finite(parameters["T_amb"], "T_amb")
        ...
        if delay_steps != DELAY_STATES:
            raise ValueError("grey prediction adapter requires exactly eight delay states")
        ...
        # Reconstruct the continuous physical model from the frozen estimator
        # state. Controller/estimator cadence governs state updates only; every
        # calibration forecast is independently discretized at the locked
        # 25-second grey prediction step.
        if all(hasattr(estimator, name) for name in ("Ad", "Bd", "bd")):
            ...
            transition, q_gain, ambient_gain, _ = _discrete_input_gains(
                system,
                timestep_s=PREDICTION_STEP_S,
                capacitance=capacitance,
                conductance=conductance,
                theta=theta,
                delay_steps=delay_steps,
