...
class CalibrationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "min_stage_observations", _positive_int(self.min_stage_observations, "min_stage_observations")
        )
        ...
        object.__setattr__(
            ...
            _positive_int(self.min_positive_observations, "min_positive_observations"),
        )
        object.__setattr__(
            self,
            "min_negative_observations",
            _positive_int(self.min_negative_observations, "min_negative_observations"),
        )
...
class CalibrationDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.command_generation, bool)
            or not isinstance(self.command_generation, int)
            or self.command_generation < 0
        ):
...
class CalibrationCoordinator:
    ...
    def stop(self, runtime: CalibrationRuntimeContext | None = None) -> CalibrationDecision:
        decision = (
            self._terminal("stopped", self._actual_stage, ("operator_stop",))
            if self._state
            else self._last or self._terminal("stopped", None, ("not_active",))
        )
    ...
    def cancel_probe(self, reason: str, runtime: CalibrationRuntimeContext | None = None) -> CalibrationDecision:
        ...
        return self._with_provenance(
            self._terminal("safety_aborted", self._actual_stage, (reason,)), 0, "safety-cancel"
        )
        ...
        return self._with_provenance(
            self._remember(
                self._decision(
                    ...
                    self._stage,
                    self._state,
                    (self._event("paused", self._actual_stage, self._state.current_probe_q, 0.0, self._state),),
                )
            ),
            0,
            "pause",
    ...
    def resume(self, runtime: CalibrationRuntimeContext) -> CalibrationDecision:
        if self._state is None:
            return self._with_provenance(
                self._last or self._terminal("incomplete", None, ("not_started",)), 0, "resume"
            )
        reasons = self._guard_reasons(runtime)
        if reasons:
        ...
        return self._with_provenance(
            self._remember(
                self._decision(
                    ...
                    self._stage,
                    self._state,
                    (self._event("resumed", self._actual_stage, probe, probe, self._state),),
                )
            ),
            0,
            "resume",
    ...
    def advance(self, runtime: CalibrationRuntimeContext) -> CalibrationDecision:
        ...
        state = _State(
            state.command,
            state.stage_index,
            state.stage_started_s,
            state.schedule_position + 1,
            state.signed_dwell_plan,
            state.schedule,
            state.eligible_observations + 1,
            state.realized_values + (runtime.realized_q,),
            state.positive_observations + int(realized_probe > 0.0),
            state.negative_observations + int(realized_probe < 0.0),
            state.realized_probe_sum + realized_probe,
            max(state.rank_progress, runtime.rank_progress),
            max(state.coverage_progress, runtime.coverage_progress),
            state.continuous and runtime.continuous,
            0.0,
            state.clipping_error + realized_probe - state.current_probe_q,
            False,
        ...
        if state.schedule_position >= len(state.schedule):
            ...
            return self._remember(
                self._decision(
                    ...
                    self._stage,
                    state,
                    (self._event("incomplete", self._actual_stage, 0.0, 0.0, state, self._incomplete_reasons(state)),),
                )
            )
    ...
    def from_snapshot(
        ...
        config, state, last, paused = (
            snapshot.get("config"),
            snapshot.get("state"),
            snapshot.get("last"),
            snapshot.get("paused", False),
        )
    ...
    def _resume_from_coast(self, runtime: CalibrationRuntimeContext) -> CalibrationDecision:
        ...
        if (
            abs(runtime.temp_c - center) > self._config.band_tolerance_c
            or abs(runtime.target_c - center) > self._config.band_tolerance_c
        ):
        ...
        state = _State(
            self._state.command,
            self._state.stage_index,
            self._state.stage_started_s,
            0,
            self._state.signed_dwell_plan,
            self._state.schedule,
            current_probe_q=probe,
            completion_history=self._state.completion_history,
        )
        self._state = state
        return self._remember(
            self._decision(
                ...
                self._actual_stage,
                state,
                (self._event("stage_started", self._actual_stage, state.schedule[0], probe, state),),
            )
        )
    ...
    def _issue_probe(self, intended: float, runtime: CalibrationRuntimeContext, state: _State) -> CalibrationDecision:
        ...
        return self._remember(
            self._decision(
                ...
                self._stage,
                state,
                (self._event("probe_changed", self._actual_stage, intended, probe, state),),
            )
        )
    ...
    def _progress(self, state: _State) -> CalibrationProgress:
        ...
        return CalibrationProgress(
            ...
            len({round(value, 12) for value in values}),
            variance,
            state.positive_observations,
            state.negative_observations,
            state.rank_progress,
            state.coverage_progress,
            state.continuous,
            state.realized_probe_sum,
            state.clipping_error,
        )
    ...
    def _with_probe(self, state: _State, probe: float) -> _State:
        return _State(
            ...
            state.stage_index,
            state.stage_started_s,
            state.schedule_position,
            state.signed_dwell_plan,
            state.schedule,
            state.eligible_observations,
            state.realized_values,
            state.positive_observations,
            state.negative_observations,
            state.realized_probe_sum,
            state.rank_progress,
    ...
    def _decision(
        self, active: bool, probe: float, stage: str | None, state: _State, events: tuple[CalibrationEvent, ...] = ()
    ) -> CalibrationDecision:
        return CalibrationDecision(
            active, probe, stage, self._progress(state), events, self._config.band_centers_c[state.stage_index]
        )
