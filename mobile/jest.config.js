module.exports = {
  ...
  // "react-native-reanimated"` throws immediately. Mapping
  ...
  // NativeWorklets.native.ts and crashes the same way. react-native-worklets
  // ships the actual fix for this as "jest/resolver.js": it strips the
  // "native" condition when resolving anything under react-native-worklets,
  // so `.native.ts` files (NativeWorklets.native.ts) resolve to their plain,
  // non-native sibling (NativeWorklets.ts) instead -- a Jest-safe
  // implementation with no TurboModule access. This is the package's own
  // documented Jest integration point, not a project-specific workaround.
  ...
  //
  // The leading "\\.bun/" alternative is a bun-specific addition: bun's
  // content-addressable store puts every package under
  // node_modules/.bun/<name>@<version>+<hash>/node_modules/<name>/..., so
  // the *first* "node_modules/" segment a path-matching regex finds is
