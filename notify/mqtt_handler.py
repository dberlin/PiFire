...
class MqttNotificationHandler:
    def __init__(self, settings) -> None:
        ...
        try:
            ...
            self._mqttLogger = create_logger(
                "mqtt",
            ...
            grill_name = (
                "PiFire" if len(self._global_settings["grill_name"]) == 0 else self._global_settings["grill_name"]
            )
            self.default_payload = {
                "availability": [{"topic": f"{self.pifire_id}/availability"}],
    ...
    def _connect(self):
        try:
            ...
            ret = self.client.connect(host=settings["broker"], port=int(settings["port"]), keepalive=10)
    ...
    def _publish_autodiscover(self, category, topic, payload, qos=2, retain=True, properties=None):
        ...
        if ret.rc == mqtt.MQTT_ERR_SUCCESS:
            ...
            self._mqttLogger.error(
                f"Cannot publish autodiscover data for {topic} because of error {mqtt.connack_string(ret.rc)}"
            )
    ...
    def _publish(self, context, data):
        ...
        for device in data:
            if (
                (context == "devices" and device in self.DEVICE_SENSORS)
                or (context == "control" and device in self.CONTROL_SENSORS)
                or (context == "pid_config" and device in self.PID_CONFIG_SENSORS)
                or (context == "pid_cycle_data" and device in self.PID_CYCLE_TIME_SENSORS)
                or (context == "pellet")
                and device in self.HOPPER_SENSORS
                or (context == "pid" and device in self.PID_SENSORS)
                or (context == "notify_event")
                or (context == "system")
                or (context.startswith("control_notify_data") and device in self.CONTROL_NOTIFY_SENSORS)
                or (context.startswith("probe_data"))
            ):
    ...
    def notify(self, context: str, data: dict):
        ...
        try:
            ...
            if context != "control" or data["mode"] == self.last_mode:
                return
