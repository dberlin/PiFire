...
RUNTIME_GATES = ("abi", "loader", "horizons-5-through-24", "invalid-horizons", "cost-scaling", "cold-solves", "warm-solves")
...
class TimingGateError(RebuildError): pass
class PublicationError(RebuildError): pass
class PublicationCollisionError(PublicationError): pass
...
class BuildPaths:
    ...
    def for_repository(cls, value: str | Path) -> "BuildPaths":
        root = Path(value).resolve(); generated = root / "native/generated"; runtime = root / "controller/_native"
        return cls(root, generated, generated.parent / ".generated-staging", runtime, runtime / "releases", runtime / ".staging", runtime / "current", runtime / "rebuild.lock", root / "build/acados-rebuild")
...
def _identity(value: BuildInputIdentity) -> dict[str, Any]:
    raw = {"generated_manifest": value.generated_manifest, "native_source_sha256": value.native_source_sha256, "abi_version": value.abi_version, "host": value.host, "compiler": value.compiler, "cmake": value.cmake, "loader": value.loader}
...
def _build_digest(inputs: Mapping[str, Any], library: str) -> str:
    raw = json.dumps({"build_inputs": inputs, "library_sha256": library}, ensure_ascii=False, separators=(",", ":"), sort_keys=True).encode()
    return hashlib.sha256(raw).hexdigest()

def canonical_build_manifest(identity: BuildInputIdentity, *, library_sha256: str, built_at: str | None = None) -> dict[str, Any]:
    inputs = _identity(identity)
    return {"schema": 1, "build_digest": _build_digest(inputs, library_sha256), "build_inputs": inputs, "library_sha256": library_sha256, "built_at": built_at or datetime.now(UTC).isoformat().replace("+00:00", "Z")}

def _valid_manifest(value: Mapping[str, Any]) -> bool:
    inputs, library = value.get("build_inputs"), value.get("library_sha256")
    return isinstance(inputs, Mapping) and isinstance(library, str) and re.fullmatch(r"[0-9a-f]{64}", library) is not None and value.get("build_digest") == _build_digest(inputs, library)
...
def classify_staleness(installed: Mapping[str, Any] | None, expected_identity: BuildInputIdentity, *, actual_library_sha256: str | None) -> tuple[StalenessClass, ...]:
    if installed is None: return (StalenessClass.MISSING_RELEASE,)
    if not isinstance(installed, Mapping) or not _valid_manifest(installed): return (StalenessClass.INVALID_MANIFEST,)
    ...
    if not isinstance(og, Mapping) or not isinstance(osources, Mapping): return (StalenessClass.INVALID_MANIFEST,)
    oa, na = og.get("acados", {}), ng.get("acados", {})
    result: list[StalenessClass] = []
    if not isinstance(oa, Mapping) or any(oa.get(k) != na.get(k) for k in ("url", "tag", "revision")): result.append(StalenessClass.ACADOS_SOURCE)
    if not isinstance(oa, Mapping) or oa.get("recursive_dependencies") != na.get("recursive_dependencies"): result.append(StalenessClass.ACADOS_DEPENDENCIES)
    if og.get("python_generator_dependencies") != ng.get("python_generator_dependencies"): result.append(StalenessClass.GENERATOR_DEPENDENCIES)
    if any(og.get(k) != ng.get(k) for k in ("schema", "model_definitions", "solvers")): result.append(StalenessClass.GENERATED_PROVENANCE)
    if og.get("files") != ng.get("files"): result.append(StalenessClass.GENERATED_TREE)
    for reason, predicate in ((StalenessClass.NATIVE_WRAPPER, lambda p:p.startswith("native/src/")), (StalenessClass.PUBLIC_HEADER, lambda p:p.startswith("native/include/")), (StalenessClass.EXPORT_CONTROLS, lambda p:p.endswith((".exports", ".version-script"))), (StalenessClass.CMAKE_SOURCES, lambda p:p in {"CMakeLists.txt", "native/CMakeLists.txt", "native/AcadosPifireExports.cmake"}), (StalenessClass.PLATFORM_MAPPING, lambda p:p.startswith("cmake/"))):
        if _changed(osources, nsources, predicate): result.append(reason)
    for reason, key in ((StalenessClass.ABI,"abi_version"),(StalenessClass.HOST_PLATFORM,"host"),(StalenessClass.COMPILER,"compiler"),(StalenessClass.CMAKE_CONFIGURATION,"cmake"),(StalenessClass.LOADER,"loader")):
        if old.get(key) != new[key]: result.append(reason)
    if actual_library_sha256 != installed.get("library_sha256"): result.append(StalenessClass.LIBRARY)
    return tuple(result)

def validate_timing_gate(matrix: Mapping[int, Sequence[SolveSample]], *, configured_control_period_seconds: float | None, catalog_minimum_control_period_seconds: float) -> tuple[TimingEvidence, ...]:
    if tuple(sorted(matrix)) != HORIZONS: raise TimingGateError("timing evidence must cover every integer horizon 5 through 24")
    period = catalog_minimum_control_period_seconds if configured_control_period_seconds is None else configured_control_period_seconds
    if not math.isfinite(period) or period <= 0: raise TimingGateError("control period must be finite and positive")
    result = []
    for horizon in HORIZONS:
        samples = tuple(matrix[horizon])
        if len(samples) != SOLVES_PER_HORIZON: raise TimingGateError(f"horizon {horizon} requires exactly 1,000 solves")
        if any(not math.isfinite(s.elapsed_seconds) or s.elapsed_seconds < 0 for s in samples): raise TimingGateError(f"horizon {horizon} timing must be finite")
        failures = [i for i,s in enumerate(samples) if s.status != 0]
        if len(failures) > 5: raise TimingGateError(f"horizon {horizon} permits at most five failures")
        consecutive = sum(b == a + 1 for a,b in zip(failures, failures[1:], strict=False))
        if consecutive: raise TimingGateError(f"horizon {horizon} has consecutive failures")
        for index in failures:
            if index + 1 == len(samples): raise TimingGateError(f"horizon {horizon} failure has no recovery")
            recovery = samples[index + 1]
            if recovery.status != 0 or not recovery.result_is_finite: raise TimingGateError(f"horizon {horizon} requires finite successful recovery")
        if any(s.status == 0 and not s.result_is_finite for s in samples): raise TimingGateError(f"horizon {horizon} successful result must be finite")
        elapsed = sorted(s.elapsed_seconds for s in samples); p99 = elapsed[math.ceil(.99 * len(elapsed))-1]; maximum = elapsed[-1]
        if maximum >= period: raise TimingGateError(f"horizon {horizon} maximum missed strict threshold")
        if p99 >= .2 * period: raise TimingGateError(f"horizon {horizon} p99 missed strict threshold")
        result.append(TimingEvidence(horizon,len(samples),len(failures),consecutive,"finite-success",p99,maximum,.2*period,period))
    return tuple(result)
...
def publish_immutable_release(staging: Path, runtime_root: Path, *, filesystem: PublicationFilesystem, select: bool=True) -> Path:
    releases = runtime_root / "releases"
    if filesystem.device_id(staging) != filesystem.device_id(releases): raise PublicationError("staging and releases must be on the same filesystem")
    try:
        digest, content = filesystem.verify_complete_contained(staging); release = releases / digest
        filesystem.fsync_staged_files(staging); filesystem.fsync_directory(staging)
        try: filesystem.rename_noreplace(staging, release)
        except FileExistsError as error:
            if filesystem.existing_release_bytes(release) != content: raise PublicationCollisionError(f"conflicting immutable release: {release}") from error
            filesystem.discard_stage(staging)
        filesystem.seal_and_verify_release(release, content)
        filesystem.fsync_directory(releases)
        if select: filesystem.atomic_replace_selector(runtime_root / "current", digest); filesystem.fsync_directory(runtime_root)
        return release
    except PublicationCollisionError: raise
    except Exception as error: raise PublicationError(f"immutable release publication failed: {error}") from error
...
def run_rebuild(mode: BuildMode, *, operations: Operations) -> RebuildResult:
    phase="lock"; stale: tuple[StalenessClass,...]=()
    try:
        with operations.build_lock():
            if mode is BuildMode.IF_NEEDED:
                phase="inspect"; stale=operations.inspect_staleness()
                if not stale: operations.emit("acados build inputs exactly match; no rebuild needed"); return RebuildResult(False,operations.current_release,())
            phase="configure-fetch"; operations.configure_and_fetch(); generated=operations.paths.generated_target
            if mode is BuildMode.FULL:
                phase="stage:generated"; generated=operations.create_generated_staging()
                command=("uv","run","--no-default-groups","--group","codegen","python","-m","controller.acados.codegen.cli","--stage",str(generated))
                phase="generate"; operations.generate(command,generated)
                for gate in GENERATED_GATES: phase=f"gate:{gate}"; operations.validate_generated(gate,generated)
            phase="stage:runtime"; runtime=operations.create_runtime_staging(); phase="compile"; operations.compile_native(generated,runtime)
            for gate in RUNTIME_GATES: phase=f"gate:{gate}"; operations.validate_runtime(gate,runtime)
            phase="gate:timing"
            for row in operations.run_timing_gate(runtime,horizons=HORIZONS,solves_per_horizon=SOLVES_PER_HORIZON): operations.emit(format_timing_evidence(row))
            if mode is BuildMode.FULL: phase="publish:generated"; operations.publish_generated(generated)
            phase="publish:runtime"; digest=operations.publish_runtime(runtime); phase="selector:replace"; operations.replace_selector(digest)
            return RebuildResult(True,digest,stale)
    except RebuildError: raise
    except BaseException as error: raise RebuildError(f"{phase}: {error}") from error
...
def _selector_manifest_is_consistent(
    release: Path, manifest: Mapping[str, Any]
) -> bool:
    return (
        _valid_manifest(manifest)
        and release.name == manifest.get("build_digest")
    )


def _control_periods(
    catalog_path: Path, database_path: Path
) -> tuple[float | None, float]:
    catalog = json.loads(catalog_path.read_text(encoding="utf-8"))
    rows = catalog["metadata"]["mpc"]["config"]
    period_row = next(
        row for row in rows if row.get("option_name") == "control_period"
    )
    ...
    if database_path.is_file():
        try:
            ...
            if row is not None:
                ...
                candidate = float(
                    settings["controller"]["config"]["mpc"]["control_period"]
                )
...
def _command_compiler_identity(executable: str | Path) -> dict[str, str]:
    ...
    return {
        "id": _normalize_compiler_id(
            "Clang" if "clang" in line.lower() else "GNU"
        ),
...
def _configured_compiler_identity(build_root: Path) -> dict[str, str]:
    files = tuple(
        (build_root / "CMakeFiles").glob("*/CMakeCCompiler.cmake")
    )
    if len(files) != 1:
    ...
    command_identity = _command_compiler_identity(
    ...
    return {
        ...
        "target": (
            ...
            if target_match is not None and target_match.group(1)
            else command_identity["target"]
        ),
...
def _verify_fetched_source(
    source: Path, expected_manifest: Mapping[str, Any]
) -> None:
    ...
    if (
        _git(source, "describe", "--tags", "--exact-match", "HEAD")
        != expected.get("tag")
    ):
...
def _current_identity(
    ...
    *,
    ...
    resolved_compiler = (
        _requested_compiler_identity()
        if compiler_identity is None
        else dict(compiler_identity)
    )
...
class LocalPublicationFilesystem:
    ...
    def seal_and_verify_release(
        self, release: Path, expected: bytes
    ) -> None:
        ...
        if actual != expected:
            raise PublicationCollisionError(
                f"published release changed before selection: {release}"
            )
        if release.stat().st_mode & 0o222 or any(
            path.stat().st_mode & 0o222 for path in entries
        ):
...
def _native_output_is_finite(
    output: _GreyOutput, *, expected_horizon: int
) -> bool:
    return (
        output.sequence_length == expected_horizon
        and math.isfinite(output.objective)
        and all(
            math.isfinite(output.sequence_q[index])
            and math.isfinite(output.sequence_residual[index])
...
def _objective_matches(
    solve_input: _GreyInput, output: _GreyOutput, horizon: int
) -> bool:
    ...
    for index in range(horizon):
        ...
        state = _reference_discrete_map(
            state, previous, residual, parameters
        )[:10]
        previous = residual
    expected = 0.5 * (
        running + (state[8] - solve_input.setpoint_c) ** 2
    ...
    return math.isclose(
        output.objective, expected, rel_tol=1e-8, abs_tol=1e-8
    )


def _validate_native_behavior(
    gate: str, cold: NativeEvidence, warm: NativeEvidence
) -> None:
...
class NativeSmoke:
    ...
    def create(self, horizon: int) -> tuple[int, Any]:
        ...
        status = self.library.acados_pifire_grey_create(
            ctypes.byref(config), ctypes.byref(handle)
        )
        return int(status), handle

    def solve_evidence(
        self, handle: Any, index: int, expected_horizon: int
    ) -> tuple[SolveSample, NativeEvidence]:
        ...
        status = int(
            self.library.acados_pifire_grey_solve(
                handle, ctypes.byref(value), ctypes.byref(output)
            )
        )
        elapsed = time.perf_counter() - started
        finite = _native_output_is_finite(
            output, expected_horizon=expected_horizon
        )
        sample = SolveSample(status, elapsed, finite)
        evidence = NativeEvidence(
            status=status,
            finite=finite,
            objective_matches=finite
            and _objective_matches(value, output, expected_horizon),
...
class LocalOperations:
    ...
    def inspect_staleness(self) -> tuple[StalenessClass, ...]:
        identity = _current_identity(
            self.paths.repository, self.paths.generated_target
        )
        try:
            ...
            manifest = json.loads(
                (release / "build-manifest.json").read_text(encoding="utf-8")
            )
        ...
        return classify_staleness(
            manifest, identity, actual_library_sha256=library_digest
        )
    ...
    def configure_and_fetch(self) -> None:
        ...
        _stream(
            (
                ...
                (
                    ...
                    f"{self.paths.build_root / 'bootstrap-output'}"
                ),
            ),
        ...
        configured_compiler = _configured_compiler_identity(
            self.paths.build_root
        )
        if configured_compiler != requested_compiler:
            raise RebuildError(
                "CMake configured compiler differs from requested compiler"
            )
        self._compiler_identity = configured_compiler
        _verify_fetched_source(
            self._source,
            _validated_generated_manifest(
                self.paths.repository, self.paths.generated_target
            ),
        )
        ...
        if gate == "provenance":
            for relative, digest in manifest.get("files", {}).items():
                if _sha256(destination / relative) != digest:
                    raise RebuildError(
                        f"generated provenance mismatch: {relative}"
                    )
            for relative, digest in manifest.get(
                "model_definitions", {}
            ).items():
            ...
            metadata = json.loads(
                (destination / "grey_box/pifire_grey.json").read_text(
                    encoding="utf-8"
                )
            )
            dimensions = metadata.get("dims", {})
            actual = {
                key: dimensions.get(key) for key in ("N", "nx", "nu", "np")
            }
    ...
    def compile_native(self, generated: Path, destination: Path) -> None:
        ...
        if (
            _configured_compiler_identity(self.paths.build_root)
    ...
    def validate_runtime(self, gate: str, destination: Path) -> None:
        ...
        if gate == "abi":
            ...
            for horizon in HORIZONS:
                ...
                try:
                    sample = (
                        smoke.solve(handle, horizon, horizon)
                        if handle
                        else None
                    )
                    if (
                        ...
                        or sample is None
                        or sample.status != 0
                        or not sample.result_is_finite
                    ):
            ...
            for horizon in (4, 25):
                ...
                try:
                    if status != 1 or handle:
                        raise RebuildError(
                            f"invalid horizon {horizon} was accepted"
                        )
                finally:
                    smoke.destroy(handle)
        ...
        for horizon in horizons:
            ...
            if status != 0 or not handle:
                raise TimingGateError(
                    f"timing create failed at horizon {horizon}"
                )
            try:
