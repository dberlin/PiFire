...
def _settings_display(settings, control, controller, event):
...
def _settings_probe_select(settings, control, controller, event):
    response = request.form
...
def _settings_probe_config(settings, control, controller, event):
    probe_info = None
...
def _settings_probe_config_save(settings, control, controller, event):
...
def _settings_notify(settings, control, controller, event):
    response = request.form
    ...
    if "wled_idle_brightness" in response:
        settings["notify_services"]["wled"]["suggested_config"]["idle_brightness"] = max(
            1, min(100, int(response["wled_idle_brightness"]))
        )
    if "wled_led_count" in response:
        settings["notify_services"]["wled"]["suggested_config"]["led_count"] = max(
            1, min(1000, int(response["wled_led_count"]))
        )
...
def _settings_editprofile(settings, control, controller, event):
    response = request.form
        ...
        try:
            ...
            for item in settings["probe_settings"]["probe_map"]["probe_info"]:
                if item["profile"]["id"] == UniqueID:
                    event["type"] = "error"
                    event["text"] = (
                        f"Error: Cannot delete this profile, as it is selected for a probe.  Go to the probe settings tab and select a different profile for {item['name']}.  Then try to delete this profile again."
                    )
            if event["type"] != "error":
            ...
            try:
                ...
                for index, probe in enumerate(settings["probe_settings"]["probe_map"]["probe_info"]):
                    if probe["profile"]["id"] == UniqueID:
                        settings["probe_settings"]["probe_map"]["probe_info"][index]["profile"] = settings[
                            "probe_settings"
                        ]["probe_profiles"][UniqueID]
                        profile_in_use = True
...
def _settings_addprofile(settings, control, controller, event):
    response = request.form
...
def _settings_controller_card(settings, control, controller, event):
    response = request.form
...
def _settings_startup(settings, control, controller, event):
    response = request.form
    ...
    if is_not_blank(response, "pwm_duty_cycle"):
        ...
        settings["startup"]["pwm_duty_cycle"] = max(
            settings["startup"]["pwm_duty_cycle"], settings["pwm"]["min_duty_cycle"]
        )
...
def _settings_history(settings, control, controller, event):
    response = request.form
...
def _settings_safety(settings, control, controller, event):
    response = request.form
...
def _settings_pellets(settings, control, controller, event):
    response = request.form
...
def _settings_smartstart_post(settings, control, controller, event):
...
def _settings_pwm_duty_cycle_get(settings, control, controller, event):
    temps = settings["pwm"]["temp_range_list"]
    profiles = settings["pwm"]["profiles"]
    return jsonify({"dc_temps_list": temps, "dc_profiles": profiles})


def _settings_pwm_duty_cycle_post(settings, control, controller, event):
    response = request.json
...
_SETTINGS_DISPATCH = {
    ("POST", "dashboard_config"): _settings_dashboard_config,
    ("POST", "display"): _settings_display,
    ("POST", "probe_select"): _settings_probe_select,
    ("POST", "probe_config"): _settings_probe_config,
    ("POST", "probe_config_save"): _settings_probe_config_save,
    ("POST", "notify"): _settings_notify,
    ("POST", "editprofile"): _settings_editprofile,
    ("POST", "addprofile"): _settings_addprofile,
    ("POST", "controller_card"): _settings_controller_card,
    ("POST", "cycle"): _settings_cycle,
    ("POST", "pwm"): _settings_pwm,
    ("POST", "startup"): _settings_startup,
    ("POST", "history"): _settings_history,
    ("POST", "safety"): _settings_safety,
    ("POST", "pellets"): _settings_pellets,
    ("GET", "smartstart"): _settings_smartstart_get,
    ("POST", "smartstart"): _settings_smartstart_post,
    ("GET", "pwm_duty_cycle"): _settings_pwm_duty_cycle_get,
    ("POST", "pwm_duty_cycle"): _settings_pwm_duty_cycle_post,
}
