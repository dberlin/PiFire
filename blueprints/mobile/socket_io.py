...
def _pellets_load_profile(pelletdb, action_data):
...
def _pellets_hopper_check(pelletdb, action_data):
    control = read_control()
    control["hopper_check"] = True
    write_control(control, WriteKind.MERGE, origin="app-socketio")
    return _response(result="OK")
...
def _pellets_edit_brands(pelletdb, action_data):
...
def _pellets_edit_woods(pelletdb, action_data):
...
def _pellets_add_profile(pelletdb, action_data):
...
def _pellets_edit_profile(pelletdb, action_data):
    if "profile" in action_data:
...
def _pellets_delete_profile(pelletdb, action_data):
    if "profile" in action_data:
        profile_id = action_data["profile"]
...
def _pellets_delete_log(pelletdb, action_data):
...
_PELLETS_DISPATCH = {
    "load_profile": _pellets_load_profile,
    "hopper_check": _pellets_hopper_check,
    "edit_brands": _pellets_edit_brands,
    "edit_woods": _pellets_edit_woods,
    "add_profile": _pellets_add_profile,
    "edit_profile": _pellets_edit_profile,
    "delete_profile": _pellets_delete_profile,
    "delete_log": _pellets_delete_log,
}


def _post_app_data_pellets(settings, type, request):
    pelletdb = read_pellets_store()
    handler = _PELLETS_DISPATCH.get(type)
    if handler is None:
        return _response(result="Error", message="Error: Received request without valid type")
