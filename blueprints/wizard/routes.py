...
def _wizard_installstatus(settings, control, wizardData, python_exec):
...
def _wizard_finish(settings, control, wizardData, python_exec):
    r = request.form
    ...
    # control["mode"] != Mode.STOP: fall through to the tail render, which
    # appends the "cannot be run while the system is active" error.
    return None


def _wizard_modulecard(settings, control, wizardData, python_exec):
    r = request.form
    module = r["module"]
    section = r["section"]
...
def _wizard_bt_scan(settings, control, wizardData, python_exec):
    r = request.form
    itemID = r["itemID"]
    bt_data = []
    error = None
...
def _wizard_thermoworks_discover(settings, control, wizardData, python_exec):
    r = request.form
    email = r.get("email", "")
...
def _wizard_i2c_bus_scan(settings, control, wizardData, python_exec):
    r = request.form
    itemID = r["itemID"]
    kind = r.get("kind", "")
    groups = []
    ...
    try:
        if kind == "extended":
            ...
            by_bus = [{"value": str(a["bus_num"]), "label": f"i2c-{a['bus_num']} ({a['name']})"} for a in adapters]
            by_serial = sorted(
                (
            ...
            items = [
                {"value": d["serial"], "label": f"MCP2221 serial {d['serial']}"} for d in discover_mcp2221_devices()
            ]
            if items:
...
def _wizard_usb_serial_scan(settings, control, wizardData, python_exec):
    r = request.form
    itemID = r["itemID"]
    vid_raw = r.get("vid", "")
    pid_raw = r.get("pid", "")
...
# Handlers return a Response to short-circuit, or None to fall through to the
# tail wizard.html render (the Task-1..6 dispatch contract). `finish` returns
# None when the system is active so the tail's active-system error still shows.
_WIZARD_DISPATCH = {
    ("GET", "installstatus"): _wizard_installstatus,
    ("POST", "cancel"): _wizard_cancel,
    ("POST", "finish"): _wizard_finish,
    ("POST", "modulecard"): _wizard_modulecard,
    ("POST", "bt_scan"): _wizard_bt_scan,
    ("POST", "thermoworks_discover"): _wizard_thermoworks_discover,
    ("POST", "i2c_bus_scan"): _wizard_i2c_bus_scan,
    ("POST", "usb_serial_scan"): _wizard_usb_serial_scan,
}
...
def wizard_page(action=None):
    ...
    handler = _WIZARD_DISPATCH.get((request.method, action))
    if handler is not None:
        result = handler(settings, control, wizardData, python_exec)
