...
def _probeconfig_devices_delete_device(r, settings, wizardData, wizardInstallInfo, alerts):
...
def _probeconfig_devices_add_config(r, settings, wizardData, wizardInstallInfo, alerts):
...
def _probeconfig_devices_add_device(r, settings, wizardData, wizardInstallInfo, alerts):
    errors = 0
    ...
    if r["name"] == "":
        alert = {"message": "Device name is blank.  Please select a unique device name.", "type": "error"}
        alerts.append(alert)
        errors += 1
...
def _probeconfig_devices_edit_config(r, settings, wizardData, wizardInstallInfo, alerts):
    """Populate Configuration Settings into Modal"""
    device_name = r["name"]
    for index, device in enumerate(wizardInstallInfo["probe_map"]["probe_devices"]):
        if device["device"] == device_name:
...
def _probeconfig_devices_edit_device(r, settings, wizardData, wizardInstallInfo, alerts):
    errors = 0
...
def _probeconfig_ports_delete_probe(r, settings, wizardData, wizardInstallInfo, alerts):
            ...
            for index, device in enumerate(wizardInstallInfo["probe_map"]["probe_devices"]):
                if "virtual" in device["module"]:
                    if probe["label"] in device["config"]["probes_list"]:
                        wizardInstallInfo["probe_map"]["probe_devices"][index]["config"]["probes_list"].remove(
                            probe["label"]
                        )
...
def _probeconfig_ports_config(r, settings, wizardData, wizardInstallInfo, alerts):
    defaultLabel = r["label"]
    defaultConfig = {"name": "", "device_port": "", "type": "", "profile_id": "", "enabled": "true"}
    ...
    return render_template_string(
        render_string, defaultLabel=defaultLabel, defaultConfig=defaultConfig, configOptions=configOptions
    )


def _probeconfig_ports_add_edit_probe(r, settings, wizardData, wizardInstallInfo, alerts):
    errors = 0
    new_probe = {}
    for key, config_value in r.items():
    ...
    if new_probe["name"] == "":
        ...
        alert = {"message": "Probe name is empty.  Please select a probe name.", "type": "error"}
        alerts.append(alert)
    ...
    if errors:
            ...
            if "virtual" in device["module"]:
                if r["name"] in device["config"]["probes_list"]:
                    for item, value in enumerate(
                        wizardInstallInfo["probe_map"]["probe_devices"][index]["config"]["probes_list"]
                    ):
                        if value == r["name"]:
                            wizardInstallInfo["probe_map"]["probe_devices"][index]["config"]["probes_list"][item] = (
                                new_probe["label"]
                            )
        ...
        # Other Error
        alert = {"message": "Error Adding/Editing Probe.  Please try again.", "type": "error"}
        alerts.append(alert)
    return None


_PROBECONFIG_DISPATCH = {
    ("devices", "delete_device"): _probeconfig_devices_delete_device,
    ("devices", "add_config"): _probeconfig_devices_add_config,
    ("devices", "add_device"): _probeconfig_devices_add_device,
    ("devices", "edit_config"): _probeconfig_devices_edit_config,
    ("devices", "edit_device"): _probeconfig_devices_edit_device,
    ("ports", "delete_probe"): _probeconfig_ports_delete_probe,
    ("ports", "config"): _probeconfig_ports_config,
    ("ports", "add_probe"): _probeconfig_ports_add_edit_probe,
    ("ports", "edit_probe"): _probeconfig_ports_add_edit_probe,
}
...
def probeconfig_page():
    ...
    if request.method == "GET":
        ...
        return render_template_string(
            render_string,
            probe_map=wizardInstallInfo["probe_map"],
            modules=wizardData["modules"]["probes"],
            alerts=alerts,
        )
    elif request.method == "POST":
        r = request.form
        handler = _PROBECONFIG_DISPATCH.get((r["section"], r["action"]))
        if handler is not None:
            result = handler(r, settings, wizardData, wizardInstallInfo, alerts)
