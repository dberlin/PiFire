...
Three latent bugs found and pinned (not routed around):

  1. **First point after startup is always dropped** (influxdb_handler.py:9,
     60-61): ``__init__`` sets ``self.last_updated = time.time()`` instead of
     0/None, and ``notify()``'s debounce checks
     ``time.time() - self.last_updated < 1``. The handler is constructed and
     then immediately ``notify()``-ed from the same call site
     (notify/notifications.py:534-538), so the elapsed time is essentially
     always < 1s -- the very first metrics point after every startup/handler
     re-creation is silently swallowed by the debounce, not just throttled.
     MEDIUM severity: no data loss long-term (later points get through), but
     confusing "first point missing" behavior with no logging.

  2. **Buffered points are lost on write failure, not retried**
     (influxdb_handler.py:50-53): ``buf = self.queue.copy(); self.queue.clear()``
     runs *before* ``write_api.write(...)``. If the write raises (auth
     failure, network blip, InfluxDB down -- exactly the case the bare
     ``except`` is there to handle), the just-cleared points are gone
     forever; there is no requeue. MEDIUM-HIGH severity for a metrics
     pipeline: transient InfluxDB unavailability silently drops data instead
     of buffering through it.

  3. **Hardcoded assumption of >=2 "food" probes, unguarded**
     (influxdb_handler.py:75-76): ``Probe1Key``/``Probe2Key`` index
     ``list(...)[0]``/``[1]`` into ``in_data["probe_history"]["food"]``
     unconditionally. A grill configured with 0 or 1 food probes raises
     IndexError inside ``notify()``, which has no try/except of its own, and
     neither does its only caller
     (notify/notifications.py:_send_influxdb_notification). Unlike
     mqtt_handler (which iterates the configured probe map dynamically),
     this crashes for any config other than exactly-2-food-probes. Not
     regression-tested here (would require monkeypatching the module import
     path with less value than documenting it), but confirmed by code
     reading + the field mapping test's reliance on exactly 2 food probes.
"""
...
def test_publishing_thread_write_failure_drops_buffer_and_recreates_write_api(monkeypatch):
    """Pins LATENT BUG #2: the queue is cleared before write() is attempted,
    so a write failure loses the buffered points instead of retrying them.
    ...
    sleep(1) [iter2] -> write_api #2 created (queue now empty, nothing to
    write) -> call4 try-block sleep(5) -> call5 outer sleep(1) [iter3],
    raise here."""
    monkeypatch.setattr("influxdb_client.InfluxDBClient", FakeInfluxDBClient)
    ...
    assert second_api.write_calls == []  # nothing left to write -- "lost-point" is gone
    assert handler.queue == []  # queue was cleared before the failed write; never restored
...
def test_notify_first_call_after_construction_is_dropped_by_debounce(monkeypatch):
    """Pins LATENT BUG #1: __init__ sets last_updated=time.time() (not 0),
    so a notify() called immediately after construction -- exactly the
    sequence notify/notifications.py:_send_influxdb_notification uses
    (construct handler, then notify() in the same call) -- is swallowed by
    the debounce and never queued."""
    monkeypatch.setattr(IH.threading, "Thread", FakeThread)
    handler = IH.InfluxNotificationHandler(_settings())

    handler.notify("EVT", {}, _settings(), {"current": {"hopper_level": 10}}, _in_data(), None)

    assert handler.queue == []  # the very first point is silently lost
