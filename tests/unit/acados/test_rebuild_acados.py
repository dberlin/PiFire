...
def mutate_generated(value: BuildInputIdentity, mutation: Callable[[dict[str, Any]], None]) -> BuildInputIdentity:
...
STALENESS_CASES: tuple[tuple[str, Callable[[BuildInputIdentity], BuildInputIdentity], StalenessClass], ...] = (
    (
        "acados-pin",
        lambda v: mutate_generated(v, lambda m: m["acados"].__setitem__("revision", "99" * 20)),
        StalenessClass.ACADOS_SOURCE,
    ),
    (
        "dependency-revisions",
        lambda v: mutate_generated(
            v, lambda m: m["acados"]["recursive_dependencies"].__setitem__("external/hpipm", "98" * 20)
        ),
        StalenessClass.ACADOS_DEPENDENCIES,
    ),
    (
        "generator-dependencies",
        lambda v: mutate_generated(v, lambda m: m["python_generator_dependencies"].__setitem__("casadi", "3.7.3")),
        StalenessClass.GENERATOR_DEPENDENCIES,
    ),
    (
        "generated-provenance",
        lambda v: mutate_generated(
            v, lambda m: m["model_definitions"].__setitem__("controller/acados/codegen/grey_box_ocp.py", "97" * 32)
        ),
        StalenessClass.GENERATED_PROVENANCE,
    ),
    (
        "generated-tree",
        lambda v: mutate_generated(
            v, lambda m: m["files"].__setitem__("grey_box/acados_solver_pifire_grey.c", "96" * 32)
        ),
        StalenessClass.GENERATED_TREE,
    ),
    ...
    (
        ...
        StalenessClass.CMAKE_CONFIGURATION,
    ),
    (
        "loader",
        lambda v: replace(v, loader={**v.loader, "library_filename": "libacados_pifire.dylib"}),
        StalenessClass.LOADER,
    ),
)
...
def test_library_digest_and_invalid_manifest_have_explicit_staleness_classes() -> None:
    ...
    assert classify_staleness({"build_digest": "bad"}, identity(), actual_library_sha256=None) == (
        StalenessClass.INVALID_MANIFEST,
    )
...
class Operations:
    def __init__(
        ...
        root: Path,
        *,
        stale: tuple[StalenessClass, ...] = (StalenessClass.GENERATED_TREE,),
    ...
    def configure_and_fetch(self) -> None:
        self.event("configure-fetch")

    def create_generated_staging(self) -> Path:
        self.event("stage:generated")
        return self.paths.generated_staging_root / "candidate"

    def generate(self, command: tuple[str, ...], destination: Path) -> None:
        assert destination.is_relative_to(self.paths.generated_staging_root)
        self.commands.append(command)
        self.event("generate")

    def validate_generated(self, gate: str, destination: Path) -> None:
        self.event(f"gate:{gate}")

    def create_runtime_staging(self) -> Path:
        self.event("stage:runtime")
        return self.paths.runtime_staging_root / "candidate"

    def compile_native(self, generated: Path, destination: Path) -> None:
        assert generated in (self.paths.generated_target, self.paths.generated_staging_root / "candidate")
        self.event("compile")
...
FULL_ORDER = [
    ...
    "stage:runtime",
    "compile",
    *(f"gate:{gate}" for gate in RUNTIME_GATES),
    "gate:timing",
    "publish:generated",
    "publish:runtime",
    "selector:replace",
...
def test_full_mode_runs_the_complete_locked_order_and_isolated_codegen_command() -> None:
    ...
    assert operations.commands == [
        (
            ...
            "--stage",
            str(operations.paths.generated_staging_root / "candidate"),
        )
    ]
...
def samples(
    *, count: int = 1_000, elapsed: float = 0.05, horizons: range = range(5, 25)
) -> dict[int, tuple[SolveSample, ...]]:
    return {
        h: tuple(SolveSample(status=0, elapsed_seconds=elapsed, result_is_finite=True) for _ in range(count))
        for h in horizons
    }


def test_timing_gate_covers_1000_solves_at_every_integer_horizon_with_exact_evidence() -> None:
    rows = validate_timing_gate(
        samples(), configured_control_period_seconds=1.0, catalog_minimum_control_period_seconds=0.5
    )
    assert tuple(row.horizon for row in rows) == tuple(range(5, 25))
    assert all(
        (row.solve_count, row.failure_count, row.consecutive_failure_count, row.recovery_result)
        == (1000, 0, 0, "finite-success")
        for row in rows
    )
    assert all(
        (row.p99_seconds, row.maximum_seconds, row.p99_threshold_seconds, row.maximum_threshold_seconds)
        == pytest.approx((0.05, 0.05, 0.2, 1.0))
        for row in rows
    )
    assert (
...
def test_timing_gate_rejects_failure_and_recovery_violations(
    replacements: Mapping[int, SolveSample], message: str
) -> None:
    with pytest.raises(TimingGateError, match=message):
        validate_timing_gate(
            altered(replacements), configured_control_period_seconds=1.0, catalog_minimum_control_period_seconds=0.5
        )
...
def test_timing_thresholds_are_strict(elapsed: float, message: str) -> None:
    with pytest.raises(TimingGateError, match=message):
        validate_timing_gate(
            samples(elapsed=elapsed), configured_control_period_seconds=1.0, catalog_minimum_control_period_seconds=0.5
        )


def test_settings_absence_uses_catalog_minimum_period() -> None:
    row = validate_timing_gate(
        samples(elapsed=0.09), configured_control_period_seconds=None, catalog_minimum_control_period_seconds=0.5
    )[0]
...
class PublicationFilesystem:
    def __init__(self, *, fail: str | None = None, collision: str | None = None) -> None:
        ...
        self.events: list[str] = []
        self.selected = "a" * 64
        self.releases = {self.selected: b"old"}
        self.stage_device = self.release_device = 1

    def device_id(self, path: Path) -> int:
        return self.stage_device if ".staging" in path.parts else self.release_device
    ...
    def fsync_staged_files(self, stage: Path) -> None:
        self._event("fsync-files")

    def fsync_directory(self, path: Path) -> None:
        self._event(f"fsync-dir:{path.name}")
...
def test_immutable_publication_fsyncs_before_atomic_selector_and_preserves_prior() -> None:
    ...
    selected = publish_immutable_release(
        Path("/repo/controller/_native/.staging/candidate"), Path("/repo/controller/_native"), filesystem=fs
    )
...
def test_same_release_collision_recovers_but_different_collision_never_overwrites() -> None:
    ...
    publish_immutable_release(
        Path("/repo/controller/_native/.staging/candidate"), Path("/repo/controller/_native"), filesystem=same
    )
    ...
    with pytest.raises(PublicationCollisionError):
        publish_immutable_release(
            Path("/repo/controller/_native/.staging/candidate"), Path("/repo/controller/_native"), filesystem=different
        )
...
def test_local_publication_seals_release_only_after_cross_directory_rename(tmp_path: Path) -> None:
    ...
    filesystem.seal_and_verify_release(release, filesystem.existing_release_bytes(release))
...
def test_release_collision_identity_ignores_informational_built_at(
    ...
    assert filesystem.existing_release_bytes(releases[0]) == filesystem.existing_release_bytes(releases[1])
...
def test_same_release_collision_is_sealed_before_selector_recovery(
    ...
    selected = publish_immutable_release(stage, runtime, filesystem=filesystem)
...
def test_generated_identity_recomputes_changed_tree_and_model_hashes(
    ...
    manifest = {
        ...
        "model_definitions": {"controller/acados/codegen/grey_box_ocp.py": "0" * 64},
    ...
    assert actual["files"]["grey_box/solver.c"] == hashlib.sha256(generated_file.read_bytes()).hexdigest()
    assert (
        actual["model_definitions"]["controller/acados/codegen/grey_box_ocp.py"]
...
def test_selector_release_name_must_equal_manifest_digest() -> None:
    ...
    assert rebuild_module._selector_manifest_is_consistent(Path(manifest["build_digest"]), manifest)
    assert not rebuild_module._selector_manifest_is_consistent(Path("f" * 64), manifest)
...
def test_cost_cold_and_warm_gates_check_distinct_evidence() -> None:
    ...
    with pytest.raises(RebuildError, match="cost"):
        rebuild_module._validate_native_behavior("cost-scaling", replace(cold, objective_matches=False), warm)
...
def test_active_control_period_uses_settings_and_falls_back_to_catalog(
    ...
    with sqlite3.connect(database) as connection:
        ...
        connection.execute(
            ...
            (
                "settings:general",
                json.dumps({"current": {"controller": {"config": {"mpc": {"control_period": 5.0}}}}}),
            ),
        )
...
def test_equation_parity_runs_only_in_isolated_codegen_process(
    ...
    metadata.write_text(json.dumps({"dims": {"N": 24, "nx": 11, "nu": 1, "np": 12}}))
...
def test_configure_resets_ephemeral_generated_and_output_cache_paths(
    ...
    def stream(command: tuple[str, ...], **_: object) -> None:
        ...
        (operations.paths.build_root / "acados-source-dir.txt").write_text(str(source), encoding="utf-8")
    ...
    monkeypatch.setattr(rebuild_module, "_configured_compiler_identity", lambda _path: compiler)
    ...
    command = calls[0]
    assert f"-DACADOS_PIFIRE_GENERATED_ROOT={operations.paths.generated_target}" in command
    assert (f"-DACADOS_PIFIRE_LIBRARY_OUTPUT_DIRECTORY={operations.paths.build_root / 'bootstrap-output'}") in command
