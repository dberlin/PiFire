...
def _dataset(seconds=6000.0):
    """A heat-up to a plateau then a step down -- enough excitation to identify
    the gain, the loss and the deadtime.

    The default length is what identifying this grill takes. A test asking
    something a fit passes through rather than something it determines -- that
    a held parameter is returned unchanged, say -- can pass a shorter one and
    still exercise the same path, since a solve's cost is linear in the rows it
    re-simulates on every evaluation.
    """
    t = np.arange(0.0, seconds, 5.0)
    Q = np.where(t < seconds / 2.0, 1.0, 0.2)
...
def baseline_fit():
    """`_dataset()` fitted from the shipped starting point, computed once.

    `fit_params` is deterministic, so every test that fits this dataset from
    this initial point gets the same dict; asking for it here rather than
    per-test spends one solve instead of one per question asked of it.

    Only for tests that want the UNMUTATED fit. A test that patches the fitter
    and then re-fits as its own control must call `fit_params` itself -- a
    cached value cannot re-run, so a control taken from here would assert the
    result of a solve that predates the very mutation it exists to bracket.
    """
    t, Q, temp = _dataset()
    return fit_params(t, temp, Q, T_amb=T_AMB, init=_init(), sigma=SIGMA, n_delay=N_DELAY)
...
def test_fit_recovers_the_generating_parameters(baseline_fit):
    fitted = baseline_fit
...
def test_fit_quality_is_reported_and_is_tight_on_its_own_data(baseline_fit):
    t, Q, temp = _dataset()
    fitted = baseline_fit
...
def test_the_fitted_dict_carries_every_key_the_controller_config_needs(baseline_fit):
    fitted = baseline_fit
...
def test_a_held_parameter_is_returned_exactly_as_it_was_passed():
    ...
    # A quarter of the usual cook: what is asserted below is that the fitter
    # hands back what it was handed, which no amount of extra excitation makes
    # truer -- and nine solves are run over it.
    t, Q, temp = _dataset(1500.0)
...
def test_the_reported_time_constant_accounts_for_radiative_conductance(baseline_fit):
...
def test_the_reported_sigma_is_the_one_the_fit_actually_simulated(baseline_fit):
...
def test_the_fit_reports_whether_it_converged(baseline_fit):
    t, Q, temp = _dataset()
    fitted = baseline_fit
