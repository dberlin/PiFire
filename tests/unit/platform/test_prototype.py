...
def test_init_with_dc_fan_seeds_ramp_thread_and_pwm_off():
    ...
    # (percent=0 -> 1.0), so get_output_status() reports a 0% initial fan speed
    # (fan not yet commanded on) instead of the old nonsensical raw-percent seed
    # -- see test_get_output_status_immediately_after_init_is_sane below.
...
def test_get_output_status_immediately_after_init_is_sane():
    ...
    fan_on()/set_duty_cycle() call, reports a 0% fan speed (fan not yet commanded
    on -- matching fan_off() and every other platform's initial seed) instead of
    the old nonsensical -9900.
    """
    p = GrillPlatform(_config(dc_fan=True))
    status = p.get_output_status()
    assert status["pwm"] == pytest.approx(0.0)
