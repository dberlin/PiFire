...
def test_control_snapshot_replaces_immediately_without_consuming_the_delta_queue(ds):
    ...
    control_persistence.enqueue_control_delta(control_delta(set_values={"primary_setpoint": 225}), origin="web")
...
def test_control_snapshot_copies_the_callers_mapping(ds):
    ...
    control_persistence.write_control_snapshot(snapshot, origin="control")
    snapshot["manual"]["pwm"] = 99
    snapshot["mode"] = "Stop"

    assert control_persistence.read_control() == {"mode": "Hold", "manual": {"pwm": 50}}


def test_errors_and_current_status_roundtrip(ds):
    runtime_persistence.write_errors(ErrorKind.CONTROL, ["e1"])
    assert runtime_persistence.read_errors(ErrorKind.CONTROL) == ["e1"]
    runtime_persistence.write_status({"mode": "Hold"})
    assert runtime_persistence.read_status() == {"mode": "Hold"}


def _seed_all_three():
    runtime_persistence.write_errors(ErrorKind.CONTROL, ["control banner"])
    runtime_persistence.write_errors(ErrorKind.DISPLAY, ["display banner"])
    runtime_persistence.write_errors(ErrorKind.WEB, ["web banner"])
...
def test_flushing_one_kind_leaves_the_other_two_intact(ds, flushed, survivors):
    ...
    runtime_persistence.flush_errors(flushed)

    for kind, banner in survivors:
        assert runtime_persistence.read_errors(kind) == [banner], f"flushing {flushed} erased {kind}'s banner"
    assert runtime_persistence.read_errors(flushed) == []


def test_read_all_groups_by_kind_in_declaration_order(ds):
    _seed_all_three()
    assert runtime_persistence.read_errors(ErrorKind.ALL) == ["control banner", "display banner", "web banner"]
...
def test_read_all_keeps_a_kind_in_place_when_that_kind_is_rewritten(ds):
    ...
    runtime_persistence.write_errors(ErrorKind.CONTROL, ["control first"])
    runtime_persistence.write_errors(ErrorKind.DISPLAY, ["display first"])
    assert runtime_persistence.read_errors(ErrorKind.ALL) == ["control first", "display first"]

    runtime_persistence.write_errors(ErrorKind.CONTROL, ["control rewritten"])

    assert runtime_persistence.read_errors(ErrorKind.ALL) == ["control rewritten", "display first"]


def test_all_is_a_read_only_selector(ds):
    with pytest.raises(ValueError):
        runtime_persistence.write_errors(ErrorKind.ALL, ["nope"])
    with pytest.raises(ValueError):
        runtime_persistence.flush_errors(ErrorKind.ALL)
...
def test_reading_a_fresh_database_yields_an_empty_list_for_every_kind(ds, kind):
    assert runtime_persistence.read_errors(kind) == []
...
def test_write_replaces_the_kinds_list_rather_than_appending(ds, kind):
    runtime_persistence.write_errors(kind, ["a"])
    runtime_persistence.write_errors(kind, ["b"])
    assert runtime_persistence.read_errors(kind) == ["b"]
...
def test_flush_returns_the_new_empty_state_not_the_discarded_contents(ds):
    ...
    runtime_persistence.write_errors(ErrorKind.CONTROL, ["stale banner"])
    assert runtime_persistence.flush_errors(ErrorKind.CONTROL) == []
    assert runtime_persistence.read_errors(ErrorKind.CONTROL) == []
...
def test_read_warnings_snapshot_does_not_consume(ds):
    ...
    runtime_persistence.write_warning("first")
    runtime_persistence.write_warning("second")
    snap = runtime_persistence.read_warnings_snapshot()
    assert snap["warnings"] == ["first", "second"]
    assert runtime_persistence.read_warnings_snapshot()["warnings"] == ["first", "second"]
    runtime_persistence.clear_warnings_through(snap["max_id"])
    assert runtime_persistence.read_warnings_snapshot()["warnings"] == []
...
def test_read_warnings_snapshot_max_id_matches_the_returned_strings(ds):
    ...
    runtime_persistence.write_warning("second")
    snap = runtime_persistence.read_warnings_snapshot()
    runtime_persistence.write_warning("third")  # raised after the snapshot; must outlive the clear
    ...
    runtime_persistence.clear_warnings_through(snap["max_id"])
    assert runtime_persistence.read_warnings_snapshot()["warnings"] == ["third"]
...
def test_read_probe_status_skips_unknown_type_without_raising(ds):
    ...
    runtime_persistence.write_generic_key(
        "probe_device_info",
        [
    ...
    result = runtime_persistence.read_probe_status(probe_info)  # must not raise UnboundLocalError
...
def test_read_probe_status_unknown_type_not_misfiled_after_prior_probe(ds):
    ...
    result = runtime_persistence.read_probe_status(probe_info)
