...
def test_set_then_get_wizard(monkeypatch):
    store = {}
    monkeypatch.setattr(runtime_persistence.datastore, "set_blob", lambda k, v: store.__setitem__(k, v))
    monkeypatch.setattr(runtime_persistence.datastore, "get_blob", lambda k: store.get(k))
