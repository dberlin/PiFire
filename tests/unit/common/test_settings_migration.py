...
Scope:
  ...
  - read_settings_file()/downgrade_settings()/restore_settings(): these call
    ...
    which need a live (test-isolated) SQLite datastore, and read/write a
    REAL "./backups/manifest.json" (settings_migration.py hardcodes that
    relative path rather than using common.backups.BACKUP_PATH, so unlike
    common/backups.py itself it cannot be redirected via monkeypatch --
    see the `real_backups_dir` fixture below, modeled on the equivalent
    fixture in tests/unit/bootstrap/test_startup_migration.py).
...
@pytest.fixture
def real_backups_dir():
    """downgrade_settings()/restore_settings() hardcode './backups/...'
    (relative to cwd, i.e. the repo root under a normal test run) rather
    than an injectable path, so this fixture makes the real directory
    usable for a test and restores it to its prior state afterward. It's
    gitignored -- a local scratch dir -- but other concurrent processes may
    be using it, so don't clobber anything that was already there.
    """
    ...
    for name in set(os.listdir(backups_path)) - pre_existing:
        path = os.path.join(backups_path, name)
...
def test_read_settings_file_upgrade_path_backs_up_and_upgrades(fresh, real_backups_dir):
    ...
    assert result["versions"] == d["versions"]
    assert result["globals"]["updated_message"] is True
    # backup_settings() wrote a dated backup file into ./backups/.
