...
def test_first_boot_imports_settings(fresh, monkeypatch):
    ...
    monkeypatch.setattr(settings_migration, "read_settings_file", lambda *a, **k: {"globals": {"units": "F"}})
...
def test_first_boot_idempotent(fresh, monkeypatch):
    ...
    monkeypatch.setattr(settings_migration, "read_settings_file", lambda *a, **k: {"v": 1})
    monkeypatch.setattr(backups, "read_pellet_db_file", lambda *a, **k: {"v": 1})
    datastore.init()
...
def test_first_boot_import_calls_read_settings_file_with_init_true(fresh, monkeypatch):
    ...
    monkeypatch.setattr(settings_migration, "read_settings_file", fake_read_settings_file)
    monkeypatch.setattr(backups, "read_pellet_db_file", lambda *a, **k: {})
    datastore.init()
...
def test_control_main_calls_datastore_init_before_first_settings_read():
    ...
    # Search for the actual calls as statements, not explanatory comments
    # mentioning datastore initialization or settings reads.
    init_pos = main_block.index("datastore.init()")
    first_read_pos = main_block.index("settings = store.read_settings()")
...
def test_read_pellet_db_file_corrupt_backup_does_not_recurse_infinitely(fresh, backups_dir):
    ...
    result = backups.read_pellet_db_file(filename=primary_file)
    assert result["current"]["hopper_level"] == 100
    assert result["woods"] == defaults.default_pellets()["woods"]
