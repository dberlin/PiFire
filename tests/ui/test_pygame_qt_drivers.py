"""Cover the pygame/Qt/DSI display drivers not exercised elsewhere:
  ...
  pygame_240x320.py,
  pygame_240x320b.py  -- display._base_fixed._DisplayBase subclasses (via the
                          _base_240x320 shim), part of the same 16-driver
  ...
  _base_dsi.py         -- display._base_flex.DisplayBase subclass (a "flex"
...
# ---------------------------------------------------------------------------
# pygame_240x320.py / pygame_240x320b.py -- display._base_fixed._DisplayBase
# subclasses (via the _base_240x320 shim). Same construction pattern as
...
def _drive_fixed_status_render(d):
    """Constructs+drives the status-render path shared by both _base_fixed
...
def test_pygame_240x320b_event_detect_opens_menu_and_renders_it(monkeypatch):
    ...
    with _fixed_driver_guarded(mod_320b) as (d, mock_os_system):
        ...
        try:
            d.display_surface = pygame.display.set_mode(size=(d.WIDTH, d.HEIGHT), flags=pygame.SHOWN)
            d.input_event = "ENTER"
            d._event_detect()  # dispatches to _base_fixed._menu_display("ENTER"), draws via our _display_canvas
...
# ---------------------------------------------------------------------------
# _base_dsi.py -- display._base_flex.DisplayBase subclass ("flex" driver).
...
class _FakeFlexObject:
    ...
    bodies are the same _base_flex-era code duplicated into this module)."""
...
def test_dsi_800x480t_canvas_render_path_against_real_surface(monkeypatch):
    ...
    try:
        ...
        with mock.patch.object(pygame.time, "delay"):
            d._display_splash()  # _base_flex generic splash -> our no-arg _display_canvas()
            after_splash = pygame.image.tobytes(d.display_surface, "RGB")
