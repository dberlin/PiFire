...
def test_hold_fan_assist_cycles_fan_via_pid_path():
    ...
    grill = FakeGrillPlatform(dc_fan=True)
    runner = FakeControllerRunner(period=0.01).script([ControllerUpdateResult(cycle_ratio=0.02, fan=None, input_temperature=0.0)] * 60)
    result = run_mode(
        "Hold",
        settings=settings,
        control_data=control_data,
...
def test_hold_mpc_commands_fan_suppresses_temp_profile_from_first_tick():
    ...
    runner = FakeControllerRunner(period=999, commands_fan=True).script(
        [ControllerUpdateResult(cycle_ratio=0.5, fan=None, input_temperature=0.0)] * 8  # never reaches a fan command in-window
    )
    result = run_mode(
        "Hold",
        settings=settings,
...
def test_hold_over_maxtemp_does_not_submit_controller_that_tick():
    ...
    runner = FakeControllerRunner(period=1000.0).script([ControllerUpdateResult(cycle_ratio=0.5, fan=None, input_temperature=0.0)] * 6)
    result = run_mode(
        "Hold",
        settings=settings,
        control_data=control_data,
...
def test_hold_controller_receives_current_tick_ptemp():
    ...
    runner = FakeControllerRunner(period=0.0).script([ControllerUpdateResult(cycle_ratio=0.5, fan=None, input_temperature=0.0)] * 8)
    run_mode(
        "Hold",
        settings=settings,
        control_data=control_data,
