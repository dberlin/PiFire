...
def test_open_sensor_constructs_vl53l1x_at_resolved_address_and_starts_ranging():
	...
	with mock.patch.object(tof_mod, 'busio'), mock.patch.object(tof_mod, 'board'):
		...
		try:
			VL53L1X.assert_called_once_with(tof_mod.busio.I2C.return_value, address=0x29)
...
def test_open_sensor_uses_configured_address():
	...
	with mock.patch.object(tof_mod, 'busio'), mock.patch.object(tof_mod, 'board'):
		...
		try:
			VL53L1X.assert_called_once_with(tof_mod.busio.I2C.return_value, address=0x2A)
		finally:
			_stop(hopper)
