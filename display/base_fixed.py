...
*****************************************
 ...
 Description:
   This is a base class for fixed-size (non-square) displays,
 parameterized by nominal (width, height) via subclass/shim
 class attributes _NOMINAL_WIDTH/_NOMINAL_HEIGHT/_SQUARE.
...
class DisplayBase:
    ...
    def _init_menu(self):
        ...
        self.menu["prime_selection"] = {
            "Prime_10": {"displaytext": "\u00bb10g", "icon": "10", "iconcolor": (255, 255, 255)},
            "Prime_25": {"displaytext": "\u00bb25g", "icon": "25", "iconcolor": (255, 255, 255)},
            "Prime_50": {"displaytext": "\u00bb50g", "icon": "50", "iconcolor": (255, 255, 255)},
            "Prime_10_Start": {"displaytext": "\u00bb10g & Start", "icon": "10", "iconcolor": (255, 255, 255)},
            "Prime_25_Start": {"displaytext": "\u00bb25g & Start", "icon": "25", "iconcolor": (255, 255, 255)},
            "Prime_50_Start": {"displaytext": "\u00bb50g & Start", "icon": "50", "iconcolor": (255, 255, 255)},
            "Menu_Back": {
                "displaytext": "Back",
    ...
    def _draw_text(self, text, font_name, font_point_size, text_color, rect=False, fill_color=None, outline_color=None):
        ...
        if rect:
            ...
            rect_draw.rounded_rectangle(
                (0, 0, rect_canvas_size[0], rect_canvas_size[1]),
                radius=8,
                fill=fill_color,
    ...
    def _draw_splus_icon(self, canvas, position):
        ...
        draw.text((position[0] + 2, position[1] + 6), text, font=font, fill=(100, 0, 255))
        font = ImageFont.truetype("static/font/FA-Free-Solid.otf", 24)
        text = "\uf067"  # FontAwesome Icon for PLUS
        draw.text((position[0] + 10, position[1] + 10), text, font=font, fill=(0, 0, 0))
    ...
    def _display_current(self, in_data, status_data):
        ...
        img = self._draw_gauge(img, position, size, fg_color, bg_color, percents, temps, label)
    ...
    def _menu_display(self, action):
        ...
        if self.menu["current"]["mode"] == "grill_hold_value":
            ...
            label_canvas = self._draw_text(
                str(self.menu["current"]["option"]), self.primary_font, font_point_size, (255, 255, 255)
            )
            label_origin = (
                (int(self.WIDTH // 2 - label_canvas.width // 2), int(self.HEIGHT // 3 - label_canvas.height // 2))
                if self.WIDTH == 240
                else (
                    int(self.WIDTH // 2 - label_canvas.width // 2 - 20),
                    int(self.HEIGHT // 2.5 - label_canvas.height // 2),
                )
            )
            img.paste(label_canvas, label_origin, label_canvas)
            ...
            draw.rectangle(
                [(0, (self.HEIGHT // 8) * 6), (self.WIDTH, ((self.HEIGHT // 8) * 6) + 2)], fill=(130, 130, 130)
            )
            # Draw Text
            ...
            for item in self.menu[self.menu["current"]["mode"]]:
                ...
                index += 1
            font_point_size = 80 if self.WIDTH == 240 else 120
            icon_color = self.menu[self.menu["current"]["mode"]][selected].get(
                "iconcolor", (255, 255, 255)
            )  # Get color from menu item, default to white if not defined
        ...
        label_origin = (
            (self.WIDTH - int((label_canvas.width // 2) ** 1.3)),
            int((self.HEIGHT // 2.5) - (label_canvas.height // 2) - 5),
        )
        img.paste(label_canvas, label_origin, label_canvas)
