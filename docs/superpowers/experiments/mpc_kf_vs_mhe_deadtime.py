...
class MHEController:
    ...
    def __init__(self):
        ...
        Q = m.set_variable('_u', 'Q'); Tset = m.set_variable('_tvp', 'T_set')
        ...
        mpc.setup()
        x0 = np.zeros((self.n, 1)); x0[NDELAY] = Tamb; x0[NDELAY + 1] = Tamb
        mpc.x0 = x0; mpc.set_initial_guess()
        self.mpc = mpc
        ...
        mhe.setup()
        x0m = np.zeros((self.n, 1)); x0m[NDELAY] = Tamb; x0m[NDELAY + 1] = Tamb
        mhe.x0 = x0m; mhe.set_initial_guess()
        self.mhe = mhe
...
def run_kf(deadtime, seed=0, n_minutes=120):
    ...
    return _drive(lambda y: (lambda o: (o['cycle_ratio'], o['fan']['duty']))(c.update(y)),
                  deadtime, seed, n_minutes)
...
def _drive(update_fn, deadtime, seed, n_minutes):
    ...
    for w in range(int(n_minutes * 60 / TSTEP)):
        ...
        for s in range(int(TSTEP)):
            ...
            temps.append(plant.T_c); times.append(w * TSTEP + s)
    return np.array(times), np.array(temps), np.array(solve)


def report(name, t, T, ms):
    sm = t >= 1800; e = T[sm] - SETPOINT
    print(f"  {name:14s} RMS={np.sqrt(np.mean(e**2)):4.2f}C  max={np.max(np.abs(e)):5.2f}C  "
