...
*****************************************
 ...
 That form reads the charged transport chain and the firepot as one lag chain
 whose stages are all the slowest of theirs, and it ignores the chamber's own
 warming during the coast. Both approximations are argued to err long. This
 measures how far, against a direct integration of the same grey box over
 parameter sets drawn across the promotion bounds.
...
def sample(rng):
    return dict(
        C_f=float(np.exp(rng.uniform(np.log(1.0), np.log(200.0)))),
        C_c=float(np.exp(rng.uniform(np.log(50.0), np.log(50000.0)))),
        h_fc=float(np.exp(rng.uniform(np.log(0.05), np.log(20.0)))),
