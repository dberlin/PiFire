...
def build_mpc():
    ...
    m.set_rhs('T_c', (h_fc * (T_f - T_c) - h_amb * (T_c - T_AMB_NOM)
                      - SIGMA * _rad(T_c, T_AMB_NOM) + d) / C_c)
    m.set_rhs('d', d * 0)
    m.setup()
...
def build_mhe():
    ...
    m.set_rhs('T_c', (h_fc * (T_f - T_c) - h_amb * (T_c - T_AMB_NOM)
                      - SIGMA * _rad(T_c, T_AMB_NOM) + d) / C_c, process_noise=True)
    m.set_rhs('d', d * 0, process_noise=True)
    m.set_meas('T_c_meas', T_c, meas_noise=True)
    m.setup()

    mhe = do_mpc.estimator.MHE(m, [])
    mhe.set_param(n_horizon=NH, t_step=Ts, store_full_solution=False, meas_from_data=True,
                  nlpsol_opts={'ipopt.print_level': 0, 'print_time': 0, 'ipopt.sb': 'yes'})
...
def build_truth():
    ...
    m.set_rhs('T_c', (h_fc_t * (T_f - T_c) - h_amb_t * lid * (T_c - T_amb)
                      - SIGMA_t * lid * _rad(T_c, T_amb)) / C_c_t)
    m.setup()
...
def run():
    ...
    mhe.x0 = x0; mhe.set_initial_guess()
    ...
    for k in range(N):
        ...
        log['t'].append(t); log['Tc'].append(float(sim.x0['T_c']))
        log['sp'].append(mpc_state['sp']); log['dhat'].append(xhat[2]); log['afr'].append(afr)
        mhe_ms.append((t1 - t0) * 1e3); mpc_ms.append((t2 - t1) * 1e3)
...
if __name__ == '__main__':
    ...
    print(f"d-hat range (steady)    : {L['dhat'][sm].min():+.2f} .. {L['dhat'][sm].max():+.2f}  (engaged => offset-free)")
    print(f"AFR range (steady)      : {L['afr'][sm].min():.2f} .. {L['afr'][sm].max():.2f}")
    warm = slice(11, None)
    print(f"MHE warm  : mean={mhe_ms[warm].mean():.1f} p95={np.percentile(mhe_ms[warm],95):.1f} max={mhe_ms[warm].max():.1f} ms")
    print(f"MPC warm  : mean={mpc_ms[warm].mean():.1f} ms   TOTAL/step={mhe_ms[warm].mean()+mpc_ms[warm].mean():.1f} ms")
