...
def build_mpc(direct=False):
	...
	mpc.set_param(
		n_horizon=20, t_step=Ts, store_full_solution=False, nlpsol_opts={'ipopt.print_level': 0, 'print_time': 0}
	)
	mterm = (T_c - T_set) ** 2
	lterm = (T_c - T_set) ** 2
...
Ac = np.array([[-h_fc / C_f, h_fc / C_f, 0.0], [h_fc / C_c, -(h_fc + h_amb) / C_c, 1.0 / C_c], [0.0, 0.0, 0.0]])
Baug = np.array(
	[
		[1.0 / C_f, 0.0],
...
def run(direct=False):
	...
	xhat = np.array([20.0, 20.0, 0.0])
	P = np.eye(3) * 5.0
	mpc.x0 = xhat.reshape(-1, 1)
	...
	for k in range(N):
		...
		log['sp'].append(sp)
		log['Q'].append(Qcmd if not direct else Qh)
		log['auger'].append(auger)
		log['fan'].append(fan)
		log['afr'].append(afr)
		log['eff'].append(eff)
...
def report(name, L):
	...
	if step_mask.any():
		...
		print(
			f'[setpoint step {step_from:.0f}->{step_to:.0f}C]'
	...
	if lid_mask.any():
		...
		print(f'[lid-open disturbance] max temp dip below target: {-dip:5.2f} C, recover to +-1C in {rect:4.0f} s')
