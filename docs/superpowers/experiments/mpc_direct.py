...
class DirectMPC:
    ...
    def __init__(self):
        self.sp = 100.0; self._ua = UMIN; self._uf = 1.0; self.n = ND + 3
        tau = THETA / ND
        ...
        Tf = m.set_variable('_x', 'T_f'); Tc = m.set_variable('_x', 'T_c'); d = m.set_variable('_x', 'd')
        ua = m.set_variable('_u', 'u_a'); uf = m.set_variable('_u', 'u_f')
        ...
        m.set_rhs('d', d * 0); m.setup()
        mpc = do_mpc.controller.MPC(m)
        mpc.set_param(n_horizon=24, t_step=TSTEP, store_full_solution=False,
        ...
        mpc.bounds['lower', '_u', 'u_a'] = UMIN; mpc.bounds['upper', '_u', 'u_a'] = UMAX
        mpc.bounds['lower', '_u', 'u_f'] = FMIN; mpc.bounds['upper', '_u', 'u_f'] = FMAX
        tv = mpc.get_tvp_template()
        def tvp(t):
            for k in range(25): tv['_tvp', k, 'T_set'] = self.sp
            return tv
        mpc.set_tvp_fun(tvp); mpc.setup()
        x0 = np.zeros((self.n, 1)); x0[ND] = Tamb; x0[ND + 1] = Tamb
        mpc.x0 = x0; mpc.set_initial_guess(); self.mpc = mpc

        # MHE (u_a, u_f known tvps)
        me = do_mpc.model.Model('continuous')
        q2 = [me.set_variable('_x', f'q{i}') for i in range(ND)]
        Tf2 = me.set_variable('_x', 'T_f'); Tc2 = me.set_variable('_x', 'T_c'); d2 = me.set_variable('_x', 'd')
        uaa = me.set_variable('_tvp', 'u_a'); uff = me.set_variable('_tvp', 'u_f')
        ...
        me.set_rhs('d', d2 * 0, process_noise=True); me.set_meas('y', Tc2, meas_noise=True); me.setup()
        mhe = do_mpc.estimator.MHE(me, [])
        mhe.set_param(n_horizon=10, t_step=CP, store_full_solution=False, meas_from_data=True,
                      nlpsol_opts={'ipopt.print_level': 0, 'print_time': 0, 'ipopt.sb': 'yes'})
        mhe.set_default_objective(np.diag([1.0] * (ND + 2) + [0.01]), np.array([[1 / 0.04]]),
                                  P_w=np.diag([10.0] * (ND + 2) + [0.05]))
        self.uah = deque([UMIN] * 11, maxlen=11); self.ufh = deque([1.0] * 11, maxlen=11)
        tv2 = mhe.get_tvp_template()
        def tvp2(t):
            for k in range(11):
                tv2['_tvp', k, 'u_a'] = self.uah[k]; tv2['_tvp', k, 'u_f'] = self.ufh[k]
            return tv2
        mhe.set_tvp_fun(tvp2); mhe.setup()
        x0m = np.zeros((self.n, 1)); x0m[ND] = Tamb; x0m[ND + 1] = Tamb
        mhe.x0 = x0m; mhe.set_initial_guess(); self.mhe = mhe

    def set_target(self, sp): self.sp = sp

    def update(self, y):
        self.uah.append(self._ua); self.ufh.append(self._uf)
        ...
        self._ua = float(np.clip(ua, UMIN, UMAX)); self._uf = float(np.clip(uf, FMIN, FMAX))
        return self._ua, self._uf * 100.0


class Cascade:
    def __init__(self):
        cfg = dict(_DEFAULTS); cfg.update(enable_fan_input=True, K_Q=KQ)
        self.c = Controller(cfg, 'C', {'u_min': 0.1, 'u_max': 0.9, 'HoldCycleTime': 25})
    def set_target(self, sp): self.c.set_target(sp)
    def update(self, y):
        o = self.c.update(y); return o['cycle_ratio'], o['fan']['duty']
