...
def run(enable_fan_input, fan_is_lever=True, fixed_fan=None, n_minutes=120, seed=0, deadtime=40):
    cfg = dict(
        n_horizon=20, t_step=TS, control_period=TS, Q_w=1.0, R_dQ=0.02,
        Q_min=5.0, Q_max=100.0, C_f=60.0, C_c=306.0, h_fc=2.0, h_amb=0.55,
        T_amb=20.0, fan_min_pct=40.0, fan_max_pct=100.0,
    ...
    for w in range(n_windows):
        ...
        for s in range(int(TS)):
            ...
            temps.append(plant.T_c); times.append(w * TS + s)
...
def report(name, times, temps):
    ...
    print(f"\n== {name} ==")
    print(f"  steady-state error: max |e|={np.max(np.abs(err)):5.2f}C  "
          f"RMS={np.sqrt(np.mean(err**2)):4.2f}C  bias={np.mean(err):+5.2f}C")
    print(f"  within +-1C: {100*np.mean(np.abs(err)<=1.0):4.1f}%   "
