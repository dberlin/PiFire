...
def model_deriv(x, Q):                # the controller's grey-box rhs, x=[q0..q3,T_f,T_c,d]
    ...
    dx[ND + 1] = (CFG['h_fc'] * (Tf - Tc) - CFG['h_amb'] * (Tc - CFG['T_amb'])
                  - _rad_loss(Tc, CFG['T_amb'], CFG['sigma']) + d) / CFG['C_c']
    return dx


def main():
    c = Controller(dict(CFG), 'F', CYCLE); c.set_target(225.0)
    p = GrillSim(seed=0)
    ...
    post = PT[step_sec:]
    pk = post.max(); pk_t = (np.argmax(post)) / 60.0
    ...
    for horizon_min in (10, 40):
        ...
        print(f"  over {horizon_min:2d} min: plant peak {C2F(plant_win.max()):6.1f}F  "
