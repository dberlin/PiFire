...
class MPCAdapter:
    ...
    def set_target(self, sc):
        self.c.set_target(sc)

    def update(self, yc):
        return float(np.clip(self.c.update(yc)["cycle_ratio"], 0.1, 0.9))
...
def drive(make, set_c, seed=0, minutes=90, step=None):
    ...
    for w in range(int(minutes * 60 / TS)):
        ...
        if step and not stepped and t >= step[0]:
            a.set_target(step[1])
            stepped = True  # change setpoint ONCE
