...
class NLCascade:
    def __init__(self, *, rterm=0.2, pwd=0.5, pxd=0.5, q_proc=10.0):
        self.sp = 100.0; self._lastQ = QMIN; self.n = ND + 3
        tau = THETA / ND
        ...
        mpc.set_tvp_fun(lambda t: self._sp(tv)); mpc.setup()
        ...
        mhe.set_param(n_horizon=10, t_step=CP, store_full_solution=False, meas_from_data=True,
                      nlpsol_opts={'ipopt.print_level': 0, 'print_time': 0, 'ipopt.sb': 'yes'})
        mhe.set_default_objective(np.diag([1.0] * (ND + 2) + [pxd]), np.array([[1 / 0.04]]),
                                  P_w=np.diag([q_proc] * (ND + 2) + [pwd]))
...
class LinKF:
    def __init__(self):
        cfg = dict(_DEFAULTS); cfg.update(enable_fan_input=False, K_Q=KQ)
        self.c = Controller(cfg, 'C', {'u_min': 0.1, 'u_max': 0.9, 'HoldCycleTime': 25})
    def set_target(self, sp): self.c.set_target(sp)
    def update(self, y):
        o = self.c.update(y); return o['cycle_ratio'], 100.0
...
def hold(name, ts, T, sp_c):
    ...
    print(f"  {name:30s} RMS={np.sqrt(np.mean(e**2)):4.1f}C max={np.max(np.abs(e)):4.1f}C mean={C2F(np.mean(T[sm])):3.0f}F")
...
def step(name, ts, T, st, sp2):
    ...
    print(f"  {name:30s} overshoot {max(0,np.max(Tp)-sp2):4.1f}C " + (f"settle +{s:.0f}min" if s else f"NEVER {C2F(Tp[-1]):.0f}F"))
...
if __name__ == '__main__':
    ...
    for rt in (0.2, 1.0):
        step(f"NL+MHE rterm={rt} pwd=0.5", *drive(NLCascade(rterm=rt, pwd=0.5), F2C(225), minutes=110, step=(1500, F2C(375))), 1500, F2C(375))
