...
class NonlinearMHE:
    ...
    def __init__(self, h_amb=0.30, sigma=1.4e-9):
        ...
        tv = mpc.get_tvp_template(); mpc.set_tvp_fun(lambda t: self._sp(tv)); mpc.setup()
    ...
    def update(self, y):
        ...
        try:
            ...
            Q = self._last_Q
        Q = float(np.clip(Q, QMIN, QMAX)); self._last_Q = Q
        return alloc(Q)
...
def drive(ctrl, deadtime=20, seed=0, minutes=120, step_at=None, sp2=None):
    ...
    for w in range(int(minutes * 60 / TSTEP)):
        ...
        for s in range(int(TSTEP)):
            ...
            ts.append(t + s); T.append(plant.true_Tc)
    return np.array(ts), np.array(T)


def kf_ctrl(sp=110.0):
    cfg = dict(_DEFAULTS); cfg.update(enable_fan_input=True)
...
def report(name, ts, T, sp=110.0, win=1800):
    sm = ts >= win; e = T[sm] - sp
    print(f"  {name:26s} RMS={np.sqrt(np.mean(e**2)):4.2f}  max={np.max(np.abs(e)):5.2f}  "
...
if __name__ == '__main__':
    ...
    print("\nLarge setpoint step 95 -> 130C at t=60min (steady error after step):")
    a = _KFWrap(95.0)
    ta, Ta = drive(a, minutes=120, step_at=3600, sp2=130.0)
    report("A) linear + KF", ta, Ta, sp=130.0, win=4200)
    b = NonlinearMHE(); b.set_target(95.0)
    tb, Tb = drive(b, minutes=120, step_at=3600, sp2=130.0)
    report("B) nonlinear + MHE", tb, Tb, sp=130.0, win=4200)
