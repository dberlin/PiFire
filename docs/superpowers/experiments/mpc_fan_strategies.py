...
class ProdWrap:
    def __init__(self, sp=110.0, enable_fan=True, **over):
        cfg = dict(_DEFAULTS); cfg.update(enable_fan_input=enable_fan); cfg.update(over)
...
def settle(name, ts, T, step_t=3600.0, sp=130.0, band=2.0):
    post = ts >= step_t; tt = ts[post] - step_t; Tp = T[post]; e = np.abs(Tp - sp)
    def at(m):
...
if __name__ == '__main__':
    ...
    settle("C) decoupled fan", *drive(ProdWrap(95.0, enable_fan=False), fixed_fan=FIXED_FAN_PCT, step_at=3600, sp2=130.0))
