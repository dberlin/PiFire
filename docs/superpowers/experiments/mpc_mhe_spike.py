...
def build_mhe():
    ...
    mhe.set_param(n_horizon=10, t_step=S.Ts, store_full_solution=False,
                  meas_from_data=True,
...
def run_mhe():
    ...
    sim.x0 = np.array([[20.0],[20.0]]); sim.set_initial_guess()
    ...
    for k in range(288):
        ...
        log['t'].append(t); log['Tc'].append(float(sim.x0['T_c'])); log['sp'].append(sp); log['afr'].append(afr)
...
if __name__ == '__main__':
    ...
    err = Tc - sp
    m1 = (t>=1500)&(t<2900); m2 = (t>=4600)&(t<7200); sm = m1|m2
    ...
    print(f"d-hat range (steady)    : {L['dhat'][sm].min():+.2f} .. {L['dhat'][sm].max():+.2f}   (KF used d to cancel bias)")
    print(f"Tc-hat vs true (steady) : est-true mean = {np.mean(L['Tc_hat'][sm]-L['Tc'][sm]):+.2f} C")
    mhe_t = np.array([d['mhe'] for d in ms]); mpc_t = np.array([d['mpc'] for d in ms])
    warm = slice(11, None)   # skip horizon-fill warmup
    print(f"MHE solve warm  : mean={mhe_t[warm].mean():.1f} median={np.median(mhe_t[warm]):.1f} p95={np.percentile(mhe_t[warm],95):.1f} max={mhe_t[warm].max():.1f} ms")
