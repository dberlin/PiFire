...
def make_plant(seed, *, deadtime=20, fan_lever=True, probe_tau=4.5,
               no_wind=False, no_buffer=False, match_params=False):
    ...
    if match_params:
        p.C_c = 320.0; p.T_amb = 20.0; p.h_amb0 = 0.385; p.h_fc0 = 1.0
    return p


def run(plant_factory, seed=0, settle_min=60, hold_min=60):
    cfg = dict(_DEFAULTS); cfg['est_q_dist'] = 0.05
...
if __name__ == '__main__':
    configs = [
        ...
        ("ALL off", lambda s: make_plant(s, deadtime=1, no_wind=True, probe_tau=0.5,
                                         no_buffer=True, fan_lever=False, match_params=True)),
    ]
