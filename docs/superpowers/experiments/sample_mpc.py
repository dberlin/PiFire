...
def _init_worker():
    global _MPC
    warnings.filterwarnings("ignore")
    c = Controller(dict(_DEFAULTS), 'C', dict(CYCLE)); c.set_target(SP)
    _MPC = c.mpc
...
def _solve_chunk(chunk):
    X0c, Upc = chunk
    n = X0c.shape[0]
    U0 = np.full(n, np.nan); ok = np.zeros(n, dtype=bool)
    for j in range(n):
...
def _episode(arg):
    ...
    for k in range(nsteps):
        ...
        if k >= 4:                              # let the EKF settle on warm starts
            Xh.append(np.asarray(x_hat).flatten().copy()); Up.append(lastQ); Q.append(q_exp)
...
def sample_closed_loop(episodes=120, workers=None, seed=0, minutes=60, dither=8.0, out=OUT):
    ...
    print(f"closed-loop: {episodes} episodes on {workers} workers in {dt:.0f}s "
...
def sample(n=16000, workers=None, seed=0, out=OUT):
    ...
    print(f"sampled {n} on {workers} workers in {dt:.0f}s "
