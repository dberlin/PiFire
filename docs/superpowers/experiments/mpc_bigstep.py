...
class LinMPC:
    def __init__(self, *, rterm=0.02, theta=50.0, nh=24):
        ...
        A[iTf, ND - 1] = KQ / Cf; A[iTf, iTf] = -hfc / Cf; A[iTf, iTc] = hfc / Cf
        ...
        self.xk = np.zeros(n); self.xk[iTf] = Tamb; self.xk[iTc] = Tamb; self.Pk = np.eye(n) * 5
...
def settle(name, ts, T, step_min=30, sp2_c=None, band=3.0):
    post = ts >= step_min * 60; tt = ts[post] - step_min * 60; Tp = T[post]; e = np.abs(Tp - sp2_c)
