...
class MPCGrey:
    ...
    def __init__(self, radiative):
        self.sp = 100.0; self._lastQ = QMIN; self.n = ND + 3; self.radiative = radiative
        tau = THETA / ND
        ...
        Q = m.set_variable('_u', 'Q'); Ts = m.set_variable('_tvp', 'T_set')
        ...
        mpc.set_param(n_horizon=NH, t_step=TSTEP, store_full_solution=False,
        ...
        mpc.bounds['lower', '_u', 'Q'] = QMIN; mpc.bounds['upper', '_u', 'Q'] = QMAX
        tv = mpc.get_tvp_template(); mpc.set_tvp_fun(lambda t: self._sp(tv, NH)); mpc.setup()
        ...
        if radiative:
            ...
            me.set_rhs('T_c', (hfc * (Tf2 - Tc2) - hamb * (Tc2 - Tamb)
                               - sigma * ((Tc2 + K) ** 4 - (Tamb + K) ** 4) + d2) / Cc, process_noise=True)
            ...
            tv2 = mhe.get_tvp_template(); mhe.set_tvp_fun(lambda t: self._q(tv2)); mhe.setup()
    ...
    def _build_kf(self, tau):
        n = self.n; iTf, iTc, iD = ND, ND + 1, ND + 2
        A = np.zeros((n, n))
        for i in range(ND):
            A[i, i] = -1 / tau
            if i > 0: A[i, i - 1] = 1 / tau
        A[iTf, ND - 1] = KQ / Cf
        A[iTf, iTf] = -hfc / Cf; A[iTf, iTc] = hfc / Cf
        A[iTc, iTf] = hfc / Cc; A[iTc, iTc] = -(hfc + hamb) / Cc; A[iTc, iD] = 1 / Cc
        B = np.zeros((n, 2)); B[0, 0] = 1 / tau; B[iTc, 1] = hamb * Tamb / Cc
        Mb = np.zeros((n + 2, n + 2)); Mb[:n, :n] = A; Mb[:n, n:] = B
        Md = expm(Mb * CP)
        self.Ad, self.Bd, self.bd = Md[:n, :n], Md[:n, n:n + 1], Md[:n, n + 1:n + 2]
        self.Hk = np.zeros((1, n)); self.Hk[0, iTc] = 1
        self.Qk = np.diag([1e-2] * (ND + 2) + [0.5]); self.Rk = np.array([[0.04]])
        self.xk = np.zeros(n); self.xk[iTf] = Tamb; self.xk[iTc] = Tamb
        self.Pk = np.eye(n) * 5

    def _sp(self, tv, nh):
        for k in range(nh + 1): tv['_tvp', k, 'T_set'] = self.sp
        return tv

    def _q(self, tv):
        for k in range(11): tv['_tvp', k, 'Q_app'] = self.qh[k]
        return tv
    ...
    def update(self, y):
        ...
        try:
            ...
            Q = self._lastQ
        Q = float(np.clip(Q, QMIN, QMAX)); self._lastQ = Q
        return alloc(Q)
