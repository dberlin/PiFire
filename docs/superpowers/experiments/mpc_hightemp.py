...
def steady(set_c, seed=0, minutes=70, warm=True):
    ...
    if warm:                               # skip the long climb; start near target
        p.T_c = p.T_meas = set_c; p.T_f = set_c + 60.0
    T, CR = [], []
    for w in range(int(minutes * 60 / TS)):
        out = c.update(p.measured()); r = float(np.clip(out['cycle_ratio'], 0.1, 0.9))
        CR.append(r); on = int(round(r * TS))
        for s in range(int(TS)):
            p.step(auger_on=(s < on), fan_frac=1.0); T.append(p.true_Tc)
    T = np.array(T); CR = np.array(CR)
    tail = T[len(T) // 3:]; e = tail - set_c
    crt = CR[len(CR) // 3:]
    return (np.sqrt(np.mean(e ** 2)), np.max(np.abs(e)), np.mean(e),
            np.mean(crt), np.mean(crt >= 0.895))
...
if __name__ == '__main__':
    ...
    for f in (425, 475, 525, 575, 600):
        ...
        rms = np.mean([r[0] for r in rs]); mx = np.max([r[1] for r in rs])
        bias = np.mean([r[2] for r in rs]); fire = np.mean([r[3] for r in rs])
    ...
    for f in (450, 550):
        sc = F2C(f)
        ri, ov, rms = climb(sc)
        print(f"  cold -> {f}F ({sc:.0f}C): rise {ri:5.1f}min  overshoot {ov:5.1f}F  settled RMS {rms:.2f}C", flush=True)
