...
Implemented and verified `incremental-cas/v2`, immutable parent-only source capture, picklable worker envelopes, cell-level dynamic LPT, timing-free canonical evidence, and state-space refresh candidate-exhaustion rejection. Focused runner: 11 passed; state-space: 18 passed; online tiny scenario: 1 passed; complete bakeoff package: 190 passed; Ruff passed. The full 1,080-cell run completed uniquely with 0 failures in 2m18s contaminated wall time; its gzip shard is 61,743,056 B and canonical document is 516,299,529 B. Source is `cf6028cc` (empty child) over `ea44eb60`; runtime is `not_measured`. Selection is none, with every arm on the Pareto frontier and target missed by all; state-space control scores are 29.414 (Grill) and 68.887 (MAK). Scheduled-ARX prediction RMSE is 4.690@60, 20.665@300, 98.736@3600 for simulated evidence and 6.596@60, 32.907@300 for real evidence; DMC is pathological. The run's observed throughput improved over the stopped serial 405 rows/50m29s result, but neither is qualifying timing. See `.superpowers/sdd/2026-08-04-linear-mpc-model-bakeoff/parallel-runner-final.md`.
