...
>
> **`n_lid` stayed at 24, but its meaning changed.** Before the fix all 24 lid-window samples were taken while the auger was paused, because the pause ran the whole opening. Now 12 are (`n_lid_paused`) and 12 are taken after the pause expires with the lid still open (`n_lid_released`), which is why every row also carries the count and margins split that way. Read strictly as the gate paragraph below defines it — "while the auger is paused" — the result is `0/12` at a `+2.002` closest approach. The 12 added samples are cold-chamber, full-authority states no training episode reaches, and the net stays `+32.7` inside `Q_min` on them. That is a **stronger** result than the old window's, but it is a different claim, and a reader comparing `0/24` before against `0/24` after is not comparing the same denominator.
>
...
>
> **What the corrected lid does change** is the size of the disagreement it is measuring. Isolating the lid fix — same arm (`faithful`/`applied`), same seed, same `.npz`, harness code the only difference — gives:
>
> | metric | pre-fix harness | post-fix harness | change |
> |---|---|---|---|
> | `excursion_n_lid` | 0/24 | 0/24 | unchanged |
> | `margin_min_to_q_min_lid` | +1.437 | +2.002 | widened |
> | `rms_lid_raw` | 1.201 | 5.265 | **4.4x** |
> | `max_lid_raw` | 1.618 | 10.036 | **6.2x** |
> | `rms_all_raw_warm` | 0.2086 | 0.6587 | **3.2x** |
>
> That control run is committed as `_net_vs_nlp_prefix_control.json` — the harness of commit `21ec263f` against this checkout's net, one seed. It is **not an experiment arm**; it is stored in a different shape from the arm files (a wrapper object with a `_what_this_is` header, not a bare list of rows) so it cannot be picked up as one by mistake.
>
> **The last row needs saying out loud, because the decision rule below binds on it.** `rms_all_raw_warm` is the only secondary the rule names, and under a like-for-like reading — same net, lid fix only — it *grew* 3.2x. It still clears the stated comparator: `_net_vs_nlp_baseline.json` reads 1.4359 / 1.4340 / 1.4250, and the post-fix arms read 0.8957 / 0.8951 / 0.8910 (`after`), 0.9281 / 0.9275 / 0.9260 (`ref_faithful`) and 0.6587 / 0.6597 / 0.6575 (`retrained_faithful`) — every one comfortably below. But that comparison is cross-provenance: the baseline is the pre-change arm this same note says must not be compared against the re-captured ones. **The plan says two different things here.** The reading order immediately below says `rms_all_raw_warm` "may size a change the excursion count already flagged, not overrule a clean excursion count"; the decision rule two paragraphs later says "either grew → go to Task 15". Under the first reading the net ships; under the second, taken like-for-like, it does not. This correction takes the first reading, because the growth is the window getting harder rather than the net getting worse — the lid now visits genuinely cold states and the net disagrees with the NLP more there — and because the excursion count is clean on both halves of the window. That is a choice between two readings the plan supports, not a fact the numbers settle, and it is recorded here rather than resolved silently.
>
> **Two artifacts could not be re-captured and are pre-Task-18.** `_net_vs_nlp_baseline.json` is a genuine pre-change arm (pre-`_applied_Q`, pre-`set_output`) and must stay one. `_net_vs_nlp_after_faithful.json` was measured against the net as it stood *before* Task 15 retrained it, which this checkout no longer has. Do not compare either against the three arms above.
>
> **`_net_vs_nlp_ref_faithful.json` changed `.npz` identity across its own git history, and its two versions are not a before/after pair.** It was created at `cafa587b` against the then-current net; `4053a7d0`, `752fadad` and `f7455cff` each replaced that net; Task 18 re-captured it on the third. Diffing that filename across `307e911c` shows `excursion_n_lid` going 5/5/2 → 0/0/0 and `margin_min_to_q_min_lid` going −0.50/−0.71/−0.26 → +2.00/+1.92/+1.86, which reads as the lid fix and is almost entirely three net swaps. Re-capturing it was still right — the old net cannot load on this checkout, and without it there is no current-checkout `command` arm at all — but the consequence is that **Task 14 Step 4's premise is no longer measurable from the artifacts**: no arm on the pre-retrain net exists under a corrected lid, so whether the *pre-retrain* net would still pass the gate cannot be read from any file here. "Step 4's decision stands" is a statement about the retrained net only.
>
> **Every row now records `net_path` and `net_sha256`**, so this class of confusion cannot recur: two rows separated by a retrain are no longer indistinguishable from the files themselves. All three re-captured arms carry `d6ddc3f05305…`. Adding those fields was the only change to those three files — every value already in them at `307e911c` is byte-identical after the re-capture, so the schema addition is verifiably not a re-measurement.
>
...
- **Primary: excursion count in the lid window.** The number of net *raw* outputs falling outside `[Q_min, Q_max]` over the physical lid opening — `excursion_n_lid`, denominator `n_lid`. Since Task 18 that window is longer than the actuator pause, so it is reported in three pieces: `excursion_n_lid_paused` covers the first `LidOpenPauseTime` seconds (the samples this bullet used to describe, when the pause and the opening were the same length), `excursion_n_lid_released` covers the rest of the opening, and `excursion_n_lid` is their sum. **Read the paused sub-count as the gate**, because that is the state the original comparison was calibrated on; read the released sub-count alongside it, because it is the harder extrapolation and a non-zero there is a real finding even while the gate passes. Baseline is exactly `0` on every seed, so any nonzero after-value is the predicted failure appearing, not noise. This is the decision.
