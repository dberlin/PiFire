...
>
> **Why it collapses to near-zero:** the pause pins the *command* to `u_min` too, so for 119 of the pause's 120 ticks the applied duty and the commanded duty are the same number and there is nothing for applied-duty feedback to correct. Only the single detection tick diverges. The old model manufactured a 120 s full-scale command-vs-applied divergence that production never produces.
>
> **Do not overstate this.** `+0.86%` over 5 seeds (sign consistent on all 5, range `+0.57%`..`+1.14%`) establishes that Task 14's number is dead, **not** that applied-duty feedback is harmful. The effect is negligible in either direction. What it does establish is that **`lid_open_225` is no longer evidence for or against this plan's change** — the scenario barely exercises a command-vs-applied divergence at all. The paths that do (manual override, `u_max` clamping, the sub-`u_min` reports Task 15's retraining addressed) are not covered by this matrix, so the plan carries no measured closed-loop benefit for applied-duty feedback. Task 14's Step 4 decision — *"Task 15 runs, the net is not shipped unchanged"* — rested on the **replay** excursion/RMS gate, not on the matrix, and is unaffected.
>
> **Artifact state.** `_matrix_after_mpc.json` was regenerated by Task 16 under the corrected lid model, because the previous capture (old model) against a `_matrix_baseline.json` that Task 16 re-captured (corrected model) made the Step 4 snippet below diff two different lid models: its 25 non-lid rows were bit-identical while all 5 `lid_open_225` rows read `iae 20968 -> 23618` (+12.6% .. +14.0%), an artifact of the lid model alone. **The Step 4 snippet now prints an identically zero diff on all 30 rows, and that zero is not evidence of anything**: Task 16 re-captured `_matrix_baseline.json` on a post-Task-13 checkout, so both files are now the *same* arm (applied-duty feedback live) under the *same* lid model, and the regenerated `_matrix_after_mpc.json` is row-for-row identical to `_matrix_baseline.json`'s 30 `mpc` rows. The pre-Task-13 arm no longer exists as a committed artifact. To reproduce the corrected comparison, run the matrix twice with `mpc.Controller.set_output` replaced by a no-op for the "before" arm — that restores exactly what pre-Task-13 MPC did, since `ControllerBase.set_output` was a no-op and `update()`'s own `self._applied_Q = Q` (`controller/mpc.py:401`) then feeds the estimator the command.
...
The matrix half of this snippet no longer measures anything — both files are now the same arm under the same lid model, so it prints a zero diff on all 30 rows. See the Task 16 correction at the top of this task before reading any matrix number below. The replay half is unaffected.
