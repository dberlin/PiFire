...
- **web-react stays green throughout Phase 1.** Every task in Phase 1 ends with `bun run typecheck`, `bun run lint`, and `bun run test` passing in `web-react`. These suites are the proof that moving code changed no behavior.
