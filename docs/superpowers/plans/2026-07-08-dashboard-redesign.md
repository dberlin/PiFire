...
- [ ] **Step 3: Implement** — Rebuild the layout to the design's header + 3-column structure.
  ...
  **Forward compatibility — larger-screen reuse (planned followup, do not build now):** author `DashScreen` to fill **its parent**, and treat that parent as a fixed **1280×720 design canvas** — never bind sizes/positions to the raw `Window`/screen dimensions. A future "larger screens" change then reuses this design unchanged by wrapping it in a scaled canvas: the QtQuick host sizes the `Window` to the real screen and places `DashScreen` inside `Item { width: 1280; height: 720; scale: Math.min(win.width/1280, win.height/720); anchors.centerIn: parent }`. Because QML is vector, this scales crisply to any 16:9 screen (1080p/1440p/4K) with the pins intact — the pins live in design space and scale with the canvas. Do **not** introduce proportional/relative units or GridLayout for this; uniform scale covers 16:9 reuse with zero layout rework. (Only a genuinely different **non‑16:9** target would justify a fluid reflow, which is a separate design, not reuse.) Keeping this invariant now is the whole cost of the cheap followup.
