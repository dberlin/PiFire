...
3. Add the locked `codegen` dependency group matching the generated provenance and regenerate the lock. Keep CasADi/acados-template out of deployed default dependencies; full rebuild invokes them through `uv run --group codegen`, while conditional mode never resolves or imports the group.
...
6. In full mode, invoke generation through the locked `uv run --group codegen` environment so `./rebuild-acados.sh` installs/resolves everything generation needs without a separate user command.
