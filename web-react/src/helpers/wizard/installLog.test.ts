...
import { formatInstallLog } from "./installLog";

describe("formatInstallLog", () => {
  it("replaces the logger prefix with just the clock", () => {
    expect(formatInstallLog("2026-08-01 12:00:03 +0000 | INFO | Resolved 12 packages\n")).toBe(
      "12:00:03  Resolved 12 packages\n",
    );
  });

  it("handles every level, not only INFO", () => {
    expect(formatInstallLog("2026-08-01 12:00:03 +0000 | ERROR | could not build wheel")).toBe(
  ...
  it("formats each line independently", () => {
    ...
    expect(formatInstallLog(text)).toBe("12:00:03  one\n12:00:04  two\n");
  });
  ...
  it("passes through a line the logger did not write", () => {
    ...
    expect(formatInstallLog(text)).toBe(
  ...
  it("leaves a bare pipe in the message alone", () => {
    ...
    expect(formatInstallLog("2026-08-01 12:00:03 +0000 | INFO | sh -c 'a | b'")).toBe(
      "12:00:03  sh -c 'a | b'",
    );
  });

  it("returns empty text unchanged", () => {
    expect(formatInstallLog("")).toBe("");
  });
});
