...
import type { ControllerMetadata, Settings } from "../../../../src/helpers/settings/settingsApi";
...
const meta: ControllerMetadata = {
  metadata: {
    pid: { friendly_name: "PID Standard", description: "", config: [] },
...
describe("readSelected", () => {
  it("returns the saved selection when the catalog declares it", () => {
    expect(readSelected({ controller: { selected: "mpc" } } as Settings, meta)).toBe("mpc");
  });
  ...
  it("falls back to the first controller when the saved selection is not in the catalog", () => {
    expect(readSelected({ controller: { selected: "nonesuch" } } as Settings, meta)).toBe("pid");
  });

  it("falls back to the first controller when nothing is saved", () => {
    expect(readSelected({ controller: {} } as Settings, meta)).toBe("pid");
    expect(readSelected({} as Settings, meta)).toBe("pid");
  });
  ...
  it("falls back to the first controller when the saved selection is not a string", () => {
    expect(readSelected({ controller: { selected: 3 } } as unknown as Settings, meta)).toBe("pid");
  });
  ...
  it("names no controller when there is no catalog", () => {
    expect(readSelected({ controller: { selected: "mpc" } } as Settings, null)).toBe("");
    expect(readSelected({ controller: { selected: "mpc" } } as Settings, { metadata: {} })).toBe(
      "",
    );
  });
});
